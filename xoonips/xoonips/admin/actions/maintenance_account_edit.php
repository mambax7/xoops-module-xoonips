...
function get_user_info($uid)
{
    ...
    $pHandler  = xoonips_getOrmHandler('xoonips', 'positions');
    if ($uid == 0) {
    ...
    // groups
    $grouplist = $mHandler->getGroupList(new Criteria('groupid', XOOPS_GROUP_ANONYMOUS, '!='));
    if ($uid == 0) {
        $groups = array(
            XOOPS_GROUP_USERS,
        );
    } else {
        $groups = $mHandler->getGroupsByUser($user['xoops']['uid']);
    }
    $user['groups'] = array();
    ...
    // position
    $positionlist       = $pHandler->getPositionList('s');
    $user['position']   = array();
    $user['position'][] = array(
        'posi_id'  => 0,
        'title'    => '--------------',
    ...
    foreach ($user_mailoks as $key => $name) {
        $user['user_mailok'][] = array(
            ...
            'checked'     => ($key == $user['xoops']['user_mailok']) ? 'checked' : '',
        );
    }
    return $user;
}
...
$tmpl->readTemplatesFromFile('maintenance_account_edit.tmpl.tpl');
...
$tmpl->addVar('main', 'user_viewemail_checked', ($user['xoops']['user_viewemail'] == 1) ? 'checked' : '');
...
$tmpl->addVar('main', 'attachsig_checked', ($user['xoops']['attachsig'] == 1 ? 'checked' : ''));
