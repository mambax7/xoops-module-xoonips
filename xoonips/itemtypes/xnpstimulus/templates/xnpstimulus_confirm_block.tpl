...
<{/if}>
  ...
  <tr>
    ...
    <td class="<{cycle name="oddeven"}>"><{$detail.stimulus_type.display_value}></td>
  </tr>
  <tr>
    <td class="head"><{$smarty.const._MD_XNPSTIMULUS_DEVELOPER_LABEL}></td>
    <td class="<{cycle name="oddeven"}>">
<{include file='db:xoonips_confirm_multiple_field.tpl' vars=$xnpstimulus_developer}>
    </td>
  </tr>
  <tr>
...
<input type="hidden" name="stimulus_type" value="<{$detail.stimulus_type.tpl_string}>"/>
<input type="hidden" name="developer" value="<{$detail.developer.tpl_string}>"/>
