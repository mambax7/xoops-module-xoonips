...
function xnptoolGetListBlock($item_basic)
{
    ...
    $xnptoolHandler = xoonips_getOrmCompoHandler('xnptool', 'item');
    $tpl->assign('xoonips_item', $xnptoolHandler->getTemplateVar(XOONIPS_TEMPLATE_TYPE_ITEM_LIST, $item_basic['item_id'], $myuid));

    // return as HTML
    return $tpl->fetch('db:xnptool_list_block.tpl');
}
...
function xnptoolGetDetailBlock($item_id)
{
    ...
    $detailHandler = xoonips_getOrmHandler('xnptool', 'item_detail');
    ...
    $xnptoolHandler = xoonips_getOrmCompoHandler('xnptool', 'item');
    $tpl->assign('xoonips_item', $xnptoolHandler->getTemplateVar(XOONIPS_TEMPLATE_TYPE_ITEM_DETAIL, $item_id, $myuid));

    // return as HTML
    return $tpl->fetch('db:xnptool_detail_block.tpl');
}
 ...
 * @internal param if $download_file_id download_file_id != false, let browser download this file automatically
 */
...
function xnptoolGetRegisterBlock()
{
    ...
    $tpl->assign('xnptool_developer',
                 xoonips_get_multiple_field_template_vars(xoonips_get_orm_from_post('xnptool', 'developer'), 'xnptool', 'developer'));

    // return HTML content
    return $tpl->fetch('db:xnptool_register_block.tpl');
}
...
function xnptoolGetConfirmBlock($item_id)
{
    ...
    if (xnpHasWithout($basic) || xnpHasWithout($detail) || xnpHasWithout($preview) || xnpHasWithout($attachment) || xnpHasWithout($readme)
        ...
        || xoonips_is_multiple_field_too_long($developer_objs, 'xnptool', 'developer')
    ) {
    ...
    // return HTML content
    return $tpl->fetch('db:xnptool_confirm_block.tpl');
}
...
function xnptoolInsertItem($item_id)
{
    ...
    $developerHandler = xoonips_getOrmHandler('xnptool', 'developer');
    $developer_objs = $formdata->getObjectArray('post', $developerHandler->getTableName(), $developerHandler, false);
    if (!$developerHandler->updateAllObjectsByForeignKey('tool_id', $item_id, $developer_objs)) {
        return false;
    }
...
function xnptoolUpdateItem($tool_id)
{
    ...
    $developerHandler = xoonips_getOrmHandler('xnptool', 'developer');
    $developer_objs = $formdata->getObjectArray('post', $developerHandler->getTableName(), $developerHandler, false);
    if (!$developerHandler->updateAllObjectsByForeignKey('tool_id', $tool_id, $developer_objs)) {
        return false;
    }
...
function xnptoolGetMetaInformation($item_id)
{
    ...
    $xnptoolHandler = xoonips_getOrmCompoHandler('xnptool', 'item');
    $xnptool = $xnptoolHandler->get($item_id);
...
function xnptoolGetAdvancedSearchBlock($search_var)
{
    ...
    // return HTML content
    return $tpl->fetch('db:xnptool_search_block.tpl');
}
...
function xnptoolGetDetailInformationQuickSearchQuery(&$wheres, &$join, $keywords)
{
    ...
    $join = " INNER JOIN $tool_developer_table ON ".$tool_developer_table.'.tool_id  = '.$xoopsDB->prefix('xoonips_item_basic')
                .'.item_id ';
...
{
    return xnpGetTopBlock($itemtype['name'], $itemtype['display_name'], 'images/icon_tool.gif', _MD_XNPTOOL_EXPLANATION, 'xnptool_tool_type',
                          xnptool_get_type_array());
}
...
function xnptoolGetMetadata($prefix, $item_id)
{
    ...
    if ($detail['attachment_dl_limit'] == 0) {
        $files = $fileHandler->getFilesInfo($item_id, 'tool_data');
