...
function xnpbinderGetListBlock($item_basic)
{
    ...
    $xnpbinderHandler = xoonips_getOrmCompoHandler('xnpbinder', 'item');
    $tpl->assign('xoonips_item', $xnpbinderHandler->getTemplateVar(XOONIPS_TEMPLATE_TYPE_ITEM_LIST, $item_basic['item_id'], $myuid));

    // Output in HTML.
    return $tpl->fetch('db:xnpbinder_list_block.tpl');
}
...
function xnpbinderGetDetailBlock($item_id)
{
    ...
    // Output in HTML.
    return $tpl->fetch('db:xnpbinder_detail_block.tpl');
}
...
function xnpbinderGetRegisterBlock()
{
    ...
    // Output in HTML.
    return $tpl->fetch('db:xnpbinder_register_block.tpl');
}
...
function xnpbinderGetConfirmBlock($item_id)
{
    ...
    if (xnpbinder_public_binder_has_not_public_item($formdata->getValueArray('post', 'xoonips_item_id', 'i', false),
        ...
        // Output in HTML.
        return $tpl->fetch('db:xnpbinder_confirm_block.tpl');
    }
}
...
function xnpbinderGetAdvancedSearchBlock($search_var)
{
    ...
    // return as HTML
    return $tpl->fetch('db:xnpbinder_search_block.tpl');
}
...
function xnpbinderGetExportItemId($item_id)
{
    ...
    foreach ($links as $link) {
        ...
        if ('xnpbinder' === $item_typeHandler->get('name')) {
            continue;
        }
...
{
    return array_diff(xnpbinder_extract_item_id($item_ids, OL_PRIVATE), xnpbinder_extract_item_id($item_ids, OL_GROUP_ONLY),
                      xnpbinder_extract_item_id($item_ids, OL_PUBLIC));
}
