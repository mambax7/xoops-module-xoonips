...
{
    $positionsHandler = xoonips_getOrmHandler('xoonips', 'positions');
    $posi_obj = $positionsHandler->get($posi_id);
...
{
    $cvitaesHandler = xoonips_getOrmHandler('xoonips', 'cvitaes');
    $cvitaes_objs = $cvitaesHandler->getCVs($uid);
    $cvitaes = array();
...
function _xoonips_showusers_get_count_items($uid)
{
    ...
    $nums = $item_showHandler->getCountPublications($uid);
...
function _xoonips_showusers_get_item_ids($item_type_id, $uid, $navi)
{
    ...
    $item_ids = array();
    $res = $item_basicHandler->open($criteria, 'its.item_id', true, $join);
    while ($item_basic_obj = $item_basicHandler->getNext($res)) {
        $item_ids[] = $item_basic_obj->get('item_id');
    }
    $item_basicHandler->close($res);
...
while ($row = $xoopsDB->fetchArray($res)) {
    ...
    while ($row2 = $xoopsDB->fetchArray($res2)) {
        ...
        switch ($val) {
            case 0:
                $sql3 = 'SELECT i2.item_id FROM '.$xoopsDB->prefix('xoonips_item_basic').' as i1, '.$xoopsDB->prefix('xoonips_item_show')
...
if ($sum_of_data !== 0) {
    ...
    switch ($val) {
        case 0:
            $sql = 'SELECT DISTINCT i1.item_id, i2.item_type_id FROM '.$tab_name1.' as i1, '.$tab_name2.' as i2, '.$tab_name3.' as i3, '
                   .$tab_name4.' as i4, '.$tab_name5.' as i5, '.$tab_name6.' as i6';
            $sql .= ' WHERE i1.uid='.$uid
                    .' AND i1.item_id=i2.item_id AND i2.item_id=i3.item_id AND i3.index_id=i4.index_id AND i4.uid IS NULL AND i4.gid IS NULL';
