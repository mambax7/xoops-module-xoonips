...
function xnpfilesGetListBlock($item_basic)
{
    ...
    $xnpfilesHandler = xoonips_getOrmCompoHandler('xnpfiles', 'item');
    $tpl->assign('xoonips_item', $xnpfilesHandler->getTemplateVar(XOONIPS_TEMPLATE_TYPE_ITEM_LIST, $item_basic['item_id'], $myuid));

    // return as HTML
    return $tpl->fetch('db:xnpfiles_list_block.tpl');
}
...
function xnpfilesGetDetailBlock($item_id)
{
    ...
    $xnpfilesHandler = xoonips_getOrmCompoHandler('xnpfiles', 'item');
    $tpl->assign('xoonips_item', $xnpfilesHandler->getTemplateVar(XOONIPS_TEMPLATE_TYPE_ITEM_DETAIL, $item_id, $myuid));

    // return as HTML
    return $tpl->fetch('db:xnpfiles_detail_block.tpl');
}
...
function xnpfilesGetRegisterBlock()
{
    ...
    // return as HTML
    return $tpl->fetch('db:xnpfiles_register_block.tpl');
}
...
function xnpfilesGetConfirmBlock($item_id)
{
    ...
    // return as HTML
    return $tpl->fetch('db:xnpfiles_confirm_block.tpl');
}
...
function xnpfilesGetAdvancedSearchBlock($search_var)
{
    ...
    // return as HTML
    return $tpl->fetch('db:xnpfiles_search_block.tpl');
}
...
function xnpfilesGetAttachmentMimetypeAdvancedSearchBlock($name)
{
    ...
    if (count($files) == 0) {
        $html
            = '<select name="data_file_mimetype"><option value="">Any</option>
				</select>';
    } else {
        $mimetypes = $files;
...
function xnpfilesGetAttachmentFiletypeAdvancedSearchBlock($name)
{
    ...
    if (count($files) == 0) {
        $html
            = '<select name="data_file_filetype_Any">
...
{
    return xnpGetTopBlock($itemtype['name'], $itemtype['display_name'], 'images/icon_files.gif', _MD_XNPFILES_EXPLANATION,
                          'xnpfiles_data_file_filetype', xnpfilesGetTypes());
}
