 ...
 * @param $item_id
 *
 * @return array|bool array( 'model_type' => arary( 'value' => internal name of model type,
...
function xnpmodelGetListBlock($item_basic)
{
    ...
    $xnpmodelHandler = xoonips_getOrmCompoHandler('xnpmodel', 'item');
    $tpl->assign('xoonips_item', $xnpmodelHandler->getTemplateVar(XOONIPS_TEMPLATE_TYPE_ITEM_LIST, $item_basic['item_id'], $myuid));

    // return as HTML
    return $tpl->fetch('db:xnpmodel_list_block.tpl');
}
...
function xnpmodelGetDetailBlock($item_id)
{
    ...
    $detailHandler = xoonips_getOrmHandler('xnpmodel', 'item_detail');
    ...
    $xnpmodelHandler = xoonips_getOrmCompoHandler('xnpmodel', 'item');
    $tpl->assign('xoonips_item', $xnpmodelHandler->getTemplateVar(XOONIPS_TEMPLATE_TYPE_ITEM_DETAIL, $item_id, $myuid));

    // return as HTML
    return $tpl->fetch('db:xnpmodel_detail_block.tpl');
}
...
function xnpmodelGetRegisterBlock()
{
    ...
    $tpl->assign('xnpmodel_creator',
                 xoonips_get_multiple_field_template_vars(xoonips_get_orm_from_post('xnpmodel', 'creator'), 'xnpmodel', 'creator'));

    // return as HTML
    return $tpl->fetch('db:xnpmodel_register_block.tpl');
}
...
function xnpmodelGetConfirmBlock($item_id)
{
    ...
    if (xnpHasWithout($basic) || xnpHasWithout($readme) || xnpHasWithout($rights) || xnpHasWithout($detail)
        || xoonips_is_multiple_field_too_long($creator_objs, 'xnpmodel', 'creator')
    ) {
    ...
    if (!empty($detail['creator'])) {
        ...
        $detail['creator_cnt'] = array(
            'value' => (string) fmod(count($creator), 2),
        );
    }
    // set to template
    global $xoopsTpl;
    ...
    // return as HTML
    return $tpl->fetch('db:xnpmodel_confirm_block.tpl');
}
...
function xnpmodelInsertItem($item_id)
{
    ...
    $creatorHandler = xoonips_getOrmHandler('xnpmodel', 'creator');
    $creator_objs = $formdata->getObjectArray('post', $creatorHandler->getTableName(), $creatorHandler, false);
    if (!$creatorHandler->updateAllObjectsByForeignKey('model_id', $item_id, $creator_objs)) {
        return false;
    }
...
function xnpmodelGetMetaInformation($item_id)
{
    ...
    $xnpmodelHandler = xoonips_getOrmCompoHandler('xnpmodel', 'item');
    $xnpmodel = $xnpmodelHandler->get($item_id);
...
function xnpmodelGetAdvancedSearchBlock($search_var)
{
    ...
    // return as HTML
    return $tpl->fetch('db:xnpmodel_search_block.tpl');
}
...
function xnpmodelGetDetailInformationQuickSearchQuery(&$wheres, &$join, $keywords)
{
    ...
    $join = " INNER JOIN $model_creator_table ON ".$model_creator_table.'.model_id  = '.$xoopsDB->prefix('xoonips_item_basic')
                .'.item_id ';
    $colnames = array(
...
{
    return xnpGetTopBlock($itemtype['name'], $itemtype['display_name'], 'images/icon_model.gif', _MD_XNPMODEL_EXPLANATION, 'xnpmodel_model_type',
...
function xnpmodelGetMetadata($prefix, $item_id)
{
    ...
    if ($detail['attachment_dl_limit'] == 0) {
        $files = $fileHandler->getFilesInfo($item_id, 'model_data');
