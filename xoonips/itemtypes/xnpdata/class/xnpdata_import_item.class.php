...
/**
 * Class XNPDataImportItemHandler
 */
class XNPDataImportItemHandler extends XooNIpsImportItemHandler
{
    ...
    /**
     * XNPDataImportItemHandler constructor.
     */
    public function __construct()
    {
        parent::__construct();
            ...
            case 'ITEM/DETAIL':
                ...
                if (count($file_type) == 0) {
                    $this->_import_item->setErrors(E_XOONIPS_ATTR_NOT_FOUND,
                                                   'file_type_name is not found:' . $attribs['FILE_TYPE_NAME'] . $this->_get_parser_error_at());
                    break;
                }
            ...
            case 'ITEM/DETAIL/FILE':
                ...
                if (strstr($attribs['FILE_NAME'], '..')) {
                    $this->_import_item->setErrors(E_XOONIPS_ATTR_INVALID_VALUE,
                                                   'invalid file_name attribute:' . $attribs['FILE_NAME'] . $this->_get_parser_error_at());
                }
                ...
                if ($this->_file_type_attribute === 'data_file') {
                    ...
                    $this->_data_file = $fileHandler->create();
                    $this->_data_file->setFilepath($this->_attachment_dir . '/' . $attribs['FILE_NAME']);
                    $this->_data_file->set('original_file_name',
                                           $unicode->decode_utf8($attribs['ORIGINAL_FILE_NAME'], xoonips_get_server_charset(), 'h'));
                    $this->_data_file->set('mime_type', $attribs['MIME_TYPE']);
                    ...
                    $this->_import_item->setErrors(E_XOONIPS_ATTR_NOT_FOUND,
                                                   'file_type_name is not found:' . $this->_file_type_attribute . $this->_get_parser_error_at());
                    break;
                }
                break;
        }
    ...
    public function xmlEndElementHandler($parser, $name)
    {
        ...
        switch (implode('/', $this->_tag_stack)) {
            ...
            case 'ITEM/DETAIL/EXPERIMENTERS/EXPERIMENTER':
                ...
                $experimenter        = $experimenterHandler->create();

                $experimenter->set('experimenter', $unicode->decode_utf8($this->_cdata, xoonips_get_server_charset(), 'h'));
            ...
            case 'ITEM/DETAIL/EXPERIMENTER':
                ...
                $experimenters       = $this->_import_item->getVar('experimenter');
                $experimenter        = $experimenterHandler->create();
                $experimenter->set('experimenter', trim($unicode->decode_utf8($this->_cdata, xoonips_get_server_charset(), 'h')));
            ...
            case 'ITEM/DETAIL/FILE':
                ...
                if ($this->_file_type_attribute === 'data_file') {
                    ...
                    if (!$fileHandler->insert($this->_data_file)) {
                        $this->_import_item->setErrors(E_XOONIPS_DB_QUERY,
                                                       "can't insert attachment file:" . $this->_data_file->get('original_file_name')
                                                       . $this->_get_parser_error_at());
                    }
                    $this->_data_file = $fileHandler->get($this->_data_file->get('file_id'));
    ...
    public function import($item)
    {
        if ($item->getUpdateFlag()) {
            ...
            if ($item->hasDataFile()) {
                $data_file   = $item->getVar('data_file');
                $fileHandler = xoonips_getOrmHandler('xoonips', 'file');
                $clonefile   = $fileHandler->fileClone($data_file);
                $clonefile->setDirty();
