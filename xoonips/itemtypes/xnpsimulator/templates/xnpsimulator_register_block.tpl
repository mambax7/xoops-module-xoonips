<!-- begin of xnpsimulator_register_block.tpl -->

<script type="text/javascript">
//return true if all required parameters are filled
<!--
function onSubmitItemType( form1 ){
  <{$xnpsimulator_developer.table_name}>.trimFields();
  var result = true;
  if( form1.simulator_data.value == '' && form1.simulator_dataFileID.value == '' ){
    alert( "<{$smarty.const._MD_XNPSIMULATOR_SIMULATOR_FILE_REQUIRED}>" );
    result = false;
  }
  if ( <{$xnpsimulator_developer.table_name}>.getNumOfFields() == 0 ) {
    alert( "<{$smarty.const._MD_XNPSIMULATOR_DEVELOPER_REQUIRED}>" );
    result = false;
  }
  return result;
}
...
<table class="outer">
  ...
  <tr>
    <td class="head"><{$smarty.const._MD_XNPSIMULATOR_DATE_LABEL}></td>
...
<{/if}>
  ...
  <tr>
    ...
    <td class="<{cycle name="oddeven"}>">
      <select name="simulator_type">
<{html_options options=$detail.simulator_type.select selected=$detail.simulator_type.value}>
      </select>
    </td>
  </tr>
  <tr>
    <td class="head"><{$smarty.const._MD_XNPSIMULATOR_DEVELOPER_LABEL}><{$smarty.const._MD_XOONIPS_ITEM_REQUIRED_MARK}></td>
    <td class="<{cycle name="oddeven"}>">
<{include file='db:xoonips_register_multiple_field.tpl' vars=$xnpsimulator_developer}>
    </td>
  </tr>
  <tr>
  ...
  <tr>
    <td class="head"><{$smarty.const._MD_XNPSIMULATOR_SIMULATOR_FILE}><{$smarty.const._MD_XOONIPS_ITEM_REQUIRED_MARK}></td>
...
<!-- end of xnpsimulator_register_block.tpl -->
