...
$to_index_obj = $indexHandler->get($to_xid);
...
foreach ($item_ids as $item_id) {
    ...
    foreach ($index_item_link_objs as $index_item_link_obj) {
        ...
        switch ($open_level) {
            case OL_PRIVATE:
                ...
                $index_item_linkHandler->delete($index_item_link_obj);
                break;
            case OL_GROUP_ONLY:
                // join old user joined group
                $gid = $index_obj->get('gid');
                if (!$admin_xgroupHandler->isGroupMember($to_uid, $gid)) {
                    $admin_xgroupHandler->addUserToXooNIpsGroup($gid, $to_uid, false);
                }
                break;
            case OL_PUBLIC:
                // nothing to do
    ...
    $index_item_linkHandler->add($to_xid, $item_id, NOT_CERTIFIED);
    // change item owner to $to_uid
    $item_basic_obj->set('uid', $to_uid);
    $item_basicHandler->insert($item_basic_obj);
    // TODO: append change logs
    $eventHandler->recordRequestTransferItemEvent($item_id, $to_uid);
    $eventHandler->recordTransferItemEvent($item_id, $to_xid, $to_uid);
}
