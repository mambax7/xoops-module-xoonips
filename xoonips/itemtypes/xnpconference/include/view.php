...
function xnpconferenceGetMetaInformation($item_id)
{
    ...
    $xnpconferenceHandler = xoonips_getOrmCompoHandler('xnpconference', 'item');
    $xnpconference        = $xnpconferenceHandler->get($item_id);
...
function xnpconferenceGetListBlock($item_basic)
{
    ...
    $xnpconferenceHandler = xoonips_getOrmCompoHandler('xnpconference', 'item');
    $tpl->assign('xoonips_item', $xnpconferenceHandler->getTemplateVar(XOONIPS_TEMPLATE_TYPE_ITEM_LIST, $item_basic['item_id'], $myuid));

    // return as HTML
    return $tpl->fetch('db:xnpconference_list_block.tpl');
}
...
function xnpconferenceGetDetailBlock($item_id)
{
    ...
    $xnpconferenceHandler = xoonips_getOrmCompoHandler('xnpconference', 'item');
    $tpl->assign('xoonips_item', $xnpconferenceHandler->getTemplateVar(XOONIPS_TEMPLATE_TYPE_ITEM_DETAIL, $item_id, $myuid));

    // return as HTML
    return $tpl->fetch('db:xnpconference_detail_block.tpl');
}
...
function xnpconferenceGetRegisterBlock()
{
    ...
    if (isset($presentation_type)) {
        if (isset($conferenceFromYear)) {
            $tpl->assign('gmtimeFrom', mktime(0, 0, 0, $formdata->getValue('post', 'conferenceFromMonth', 'i', false, 1),
                                              $formdata->getValue('post', 'conferenceFromDay', 'i', false, 1), $conferenceFromYear));
        }
        if (isset($conferenceToYear)) {
            $tpl->assign('gmtimeTo', mktime(0, 0, 0, $formdata->getValue('post', 'conferenceToMonth', 'i', false, 1),
                                            $formdata->getValue('post', 'conferenceToDay', 'i', false, 1), $conferenceToYear));
        }
        $detail = array(
            ...
            'conference_date'   => array(
                ...
                'value' => $tpl->fetch('db:xnpconference_date.tpl'),
            ),
        );
    } else {
    ...
    $tpl->assign('xnpconference_author',
                 xoonips_get_multiple_field_template_vars(xoonips_get_orm_from_post('xnpconference', 'author'), 'xnpconference', 'author'));
    ...
    // return as HTML
    return $tpl->fetch('db:xnpconference_register_block.tpl');
}
...
function xnpconferenceGetEditBlock($item_id)
{
    ...
    if (!empty($detail['conference_from_year'])) {
        $tpl->assign('gmtimeFrom',
                     mktime(0, 0, 0, $detail['conference_from_month'], $detail['conference_from_mday'], $detail['conference_from_year']));
        $tpl->assign('gmtimeTo', mktime(0, 0, 0, $detail['conference_to_month'], $detail['conference_to_mday'], $detail['conference_to_year']));
    } else {
        $tpl->assign('gmtimeFrom', mktime(0, 0, 0, $basic2['publication_month']['value'], $basic2['publication_mday']['value'],
                                          $basic2['publication_year']['value']));
        $tpl->assign('gmtimeTo', mktime(0, 0, 0, $basic2['publication_month']['value'], $basic2['publication_mday']['value'],
                                        $basic2['publication_year']['value']));
    }
    $detail                    = array_map(array(
                                               $textutil,
                                               'html_special_chars'
                                           ), $detail);
    ...
    if (!$formdata->getValue('get', 'post_id', 's', false)) {
        $detailHandler = xoonips_getOrmHandler('xnpconference', 'item_detail');
...
function xnpconferenceGetConfirmBlock($item_id)
{
    ...
    $author_objs   = $formdata->getObjectArray('post', $authorHandler->getTableName(), $authorHandler, false);
    ...
    if (isset($conference_title)) {
        ...
        $conference_from                          = date(DATE_FORMAT, mktime(0, 0, 0, $detail['conference_from_month']['value'],
                                                                             $detail['conference_from_mday']['value'],
                                                                             $detail['conference_from_year']['value']));
        $conference_to                            = date(DATE_FORMAT, mktime(0, 0, 0, $detail['conference_to_month']['value'],
                                                                             $detail['conference_to_mday']['value'],
                                                                             $detail['conference_to_year']['value']));
        $conference_date                          = 'From: ' . $conference_from . ' To: ' . $conference_to;
    } else {
        $detail = array();
    }

    if (xnpHasWithout($basic) || xnpHasWithout($presentation_file) || xnpHasWithout($conference_paper) || xnpHasWithout($detail)
        || xoonips_is_multiple_field_too_long($author_objs, 'xnpconference', 'author')
    ) {
    ...
    $tpl->assign('attachment_dl_notify', $attachment_dl_notify);
    $tpl->assign('detail', $detail);
    $tpl->assign('conference_date', array(
        'name'  => _MD_XOONIPS_ITEM_PUBLICATION_DATE_LABEL,
    ...
    // return as HTML
    return $tpl->fetch('db:xnpconference_confirm_block.tpl');
}
...
function xnpconferenceInsertItem($item_id)
{
    ...
    $vals                 = implode('\',\'', array(
        ...
        addslashes($ar['conference_title']),
        addslashes($ar['place']),
        addslashes($ar['abstract']),
        addslashes($formdata->getValue('post', 'conferenceFromYear', 'i', false)),
        addslashes($formdata->getValue('post', 'conferenceFromMonth', 'i', false)),
        addslashes($formdata->getValue('post', 'conferenceFromDay', 'i', false)),
        addslashes($formdata->getValue('post', 'conferenceToYear', 'i', false)),
        addslashes($formdata->getValue('post', 'conferenceToMonth', 'i', false)),
        addslashes($formdata->getValue('post', 'conferenceToDay', 'i', false))
    ));
    ...
    $authorHandler = xoonips_getOrmHandler('xnpconference', 'author');
    $author_objs   = $formdata->getObjectArray('post', $authorHandler->getTableName(), $authorHandler, false);
    if (!$authorHandler->updateAllObjectsByForeignKey('conference_id', $item_id, $author_objs)) {
        return false;
    }
...
function xnpconferenceUpdateItem($item_id)
{
    ...
    $sql                  = implode(',', array(
        ...
        'conference_title' . '=\'' . addslashes($ar['conference_title']) . '\'',
        'place' . '=\'' . addslashes($ar['place']) . '\'',
        'abstract' . '=\'' . addslashes($ar['abstract']) . '\'',
        'conference_from_year' . '=\'' . addslashes($formdata->getValue('post', 'conferenceFromYear', 'i', false)) . '\'',
        'conference_from_month' . '=\'' . addslashes($formdata->getValue('post', 'conferenceFromMonth', 'i', false)) . '\'',
        'conference_from_mday' . '=\'' . addslashes($formdata->getValue('post', 'conferenceFromDay', 'i', false)) . '\'',
        'conference_to_year' . '=\'' . addslashes($formdata->getValue('post', 'conferenceToYear', 'i', false)) . '\'',
        'conference_to_month' . '=\'' . addslashes($formdata->getValue('post', 'conferenceToMonth', 'i', false)) . '\'',
        'conference_to_mday' . '=\'' . addslashes($formdata->getValue('post', 'conferenceToDay', 'i', false)) . '\''
    ));
...
function xnpconferenceGetDetailInformationQuickSearchQuery($wheres, &$join, $keywords)
{
    ...
    $join   = " join $conference_author_table on " . $conference_author_table . '.conference_id  = ' . $xoopsDB->prefix('xoonips_item_basic')
              . '.item_id ';
    $wheres = xnpGetKeywordsQueries(array(
                                        "$conference_table.conference_title",
                                        "$conference_table.place",
                                        "$file_table.caption",
                                        "$conference_author_table.author"
                                    ), $keywords);
    return true;
}
...
function xnpconferenceGetAdvancedSearchBlock($search_var)
{
    ...
    // return as HTML
    return $tpl->fetch('db:xnpconference_search_block.tpl');
}
...
function xnpconferenceExportItem($export_path, $fhdl, $item_id, $attachment)
{
    ...
    if (!fwrite($fhdl, "<detail id=\"${item_id}\" version=\"1.02\">\n" . '<conference_from_year>' . $detail->getVar('conference_from_year', 's')
                       . "</conference_from_year>\n" . '<conference_from_month>' . $detail->getVar('conference_from_month', 's')
                       . "</conference_from_month>\n" . '<conference_from_mday>' . $detail->getVar('conference_from_mday', 's')
                       ...
                       . $detail->getVar('conference_to_mday', 's') . "</conference_to_mday>\n" . '<presentation_type>'
                       . $detail->getVar('presentation_type', 's') . "</presentation_type>\n" . '<conference_title>'
                       . $detail->getVar('conference_title', 's') . "</conference_title>\n" . '<place>' . $detail->getVar('place', 's') . "</place>\n"
                       . "<authors>{$authors}</authors>\n" . '<abstract>' . $detail->getVar('abstract', 's') . "</abstract>\n"
                       . '<attachment_dl_limit>' . (int)$detail->get('attachment_dl_limit') . "</attachment_dl_limit>\n" . '<attachment_dl_notify>'
...
function xnpconferenceGetModifiedFields($item_id)
{
    ...
    if ($detail) {
        foreach (array(
                     'presentation_type' => _MD_XNPCONFERENCE_PRESENTATION_TYPE_LABEL,
                     'conference_title'  => _MD_XNPCONFERENCE_CONFERENCE_TITLE_LABEL,
                     'place'             => _MD_XNPCONFERENCE_PLACE_LABEL,
                     'abstract'          => _MD_XNPCONFERENCE_ABSTRACT_LABEL
                 ) as $k => $v
        ) {
...
{
    return xnpGetTopBlock($itemtype['name'], $itemtype['display_name'], 'images/icon_conference.gif', _MD_XNPCONFERENCE_EXPLANATION,
                          'xnpconference_presentation_type', xnpconferenceGetTypes());
}
...
function xnpconferenceGetMetadata($prefix, $item_id)
{
    ...
    $authorHandler = xoonips_getOrmHandler($mydirname, 'author');
    ...
    $author_objs       = $authorHandler->getObjects($criteria);
    $detail['authors'] = array();
    foreach ($author_objs as $author_obj) {
        $detail['authors'][] = $author_obj->get('author');
    }
    $detail['conference_from_iso8601']   = xnpISO8601($detail['conference_from_year'], $detail['conference_from_month'],
    ...
    if ($detail['attachment_dl_limit'] == 0) {
        $files = $fileHandler->getFilesInfo($item_id, 'conference_file');
    ...
    $detail['conference_date'] = sprintf($fmt, $detail['conference_from_year'], $detail['conference_from_month'], $detail['conference_from_mday'],
                                         $detail['conference_to_year'], $detail['conference_to_month'], $detail['conference_to_mday'],
                                         $month_str[$detail['conference_from_month']], $month_str[$detail['conference_to_month']]);
