    ...
    if (defined('XOOPS_CUBE_LEGACY')) {
        ...
        switch ($category) {
            ...
            case XOOPS_CONF_USER:
                ...
                $configs = $configHandler->getConfigsByDirname('user'); // copy
                break;
            case XOOPS_CONF_METAFOOTER:
                ...
                //   banners
                $configs = $configHandler->getConfigsByDirname('legacyRender'); // copy
                break;
...
{
    if (preg_match('/^([0-9]{4})(-?([0-9]{2})(-?([0-9]{2})(T([0-9]{2}):([0-9]{2})(:([0-9]{2}))?(Z|([-+])([0-9]{2})([0-9]{2}))?)?)?)?$/', $str, $match)
        == 1
    ) {
        // $match[?]
        // $match[0]  : input($str)
...
function xoonips_certify_item($uid, $item_id, $index_id)
{
    ...
    if (!$index_item_linkHandler->getPerm($index_id, $item_id, $uid, 'accept')) {
        return false;
    }
    $index_item_link = $index_item_linkHandler->getByIndexIdAndItemId($index_id, $item_id);
    ...
    $event_logHandler->recordCertifyItemEvent($item_id, $index_id);
...
{
    $index_item_linkHandler = xoonips_getOrmHandler('xoonips', 'index_item_link');
    if (!$index_item_linkHandler->getPerm($index_id, $item_id, $uid, 'reject')) {
        return false;
    }
    $index_item_link = $index_item_linkHandler->getByIndexIdAndItemId($index_id, $item_id);
...
{
    $index_item_linkHandler = xoonips_getOrmHandler('xoonips', 'index_item_link');
    if (!$index_item_linkHandler->getPerm($index_id, $item_id, $uid, 'withdraw')) {
        return false;
    }
    $index_item_link = $index_item_linkHandler->getByIndexIdAndItemId($index_id, $item_id);
    if (false == $index_item_linkHandler->delete($index_item_link)) {
    ...
    $item_showHandler->deleteAll(new Criteria('item_id', $item_id));
    $item_statusHandler = xoonips_getOrmHandler('xoonips', 'item_status');
...
function xoonips_is_user_export_enabled()
{
    ...
    if ($xmemberHandler->isModerator($xoopsUser->getVar('uid'))) {
