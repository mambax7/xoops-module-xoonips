...
function xoonips_group_get_groups($uid, $gids = null)
{
    ...
    $xg_objs        =  $xgroupHandler->getGroupObjects($gids);
    $groups         = array();
    foreach ($xg_objs as $xg_obj) {
        $gid         = $xg_obj->get('gid');
        $gadmin_uids = $xgroupHandler->getUserIds($gid, true);
        $is_gadmin   = in_array($uid, $gadmin_uids);
        $gadmins     = array();
        foreach ($gadmin_uids as $gadmin_uid) {
            $u_obj = $uHandler->get($gadmin_uid);
            if (!is_object($u_obj)) {
                continue;
            }
            $gadmins[] = array(
...
function xoonips_group_get_users($gadmin_uids)
{
    ...
    $u_objs  =  $uHandler->getObjects($criteria, false, '', false, $join);
    $gadmins = array();
