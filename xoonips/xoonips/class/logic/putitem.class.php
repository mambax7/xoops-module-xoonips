...
class XooNIpsLogicPutItem extends XooNIpsLogic
{
    ...
    public function execute($vars, $response)
    {
        ...
        foreach ($related_tos as $related_to) {
            ...
            } elseif (!$detail_itemHandler->getPerm($item_id, $uid, 'read')) {
        ...
        // insert item
        $detail_itemHandler->setNew($item);
        $detail_itemHandler->setDirty($item);
        if (!$detail_itemHandler->insert($item)) {
        ...
        if (!$eventlogHandler->recordInsertItemEvent($item_id)) {
            $transaction->rollback();
