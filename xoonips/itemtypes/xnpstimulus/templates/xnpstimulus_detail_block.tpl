...
<{include file='db:xoonips_confirm_multiple_field.tpl' vars=$xoonips_item.xnpstimulus_developer}>
  ...
  <tr>
    ...
    <td class="<{cycle name="oddeven"}>"><{$stimulus_data.value}></td>
  </tr>
  <tr>
...
<{if $editable }>
<{$basic.hidden}>
<input type="hidden" name="stimulus_type" value="<{$xoonips_item.detail.stimulus_type_value}>"/>
...
<input type="hidden" name="stimulus_dataFileID" value="<{$xoonips_item.detail.stimulus_data.file_id}>"/>
<{$preview.hidden}>
<{$readme.hidden}>
<{$rights.hidden}>
<{$index.hidden}>
