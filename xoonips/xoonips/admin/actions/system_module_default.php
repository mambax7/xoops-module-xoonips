    ...
    switch ($config[$i]->getVar('conf_formtype')) {
        case 'textarea':
            if ($config[$i]->getVar('conf_valuetype') === 'array') {
                ...
                $ele = ($config[$i]->getVar('conf_value') != '') ? new XoopsFormTextArea($eletitle, $config[$i]->getVar('conf_name'),
                                                                                         $textutil->html_special_chars(implode('|',
                                                                                                                               $config[$i]->getConfValueForOutput())),
                                                                                         5, 50) : new XoopsFormTextArea($eletitle,
                                                                                                                        $config[$i]->getVar('conf_name'),
                                                                                                                        '', 5, 50);
            } else {
                $ele = new XoopsFormTextArea($eletitle, $config[$i]->getVar('conf_name'),
                                             $textutil->html_special_chars($config[$i]->getConfValueForOutput()), 5, 50);
            }
            break;
        case 'select':
            ...
            for ($j = 0; $j < $opcount; $j++) {
                ...
                $optkey
                    = defined($options[$j]->getVar('confop_name')) ? constant($options[$j]->getVar('confop_name')) : $options[$j]->getVar('confop_name');
        ...
        case 'user_multi':
            ...
            break;
        case 'password':
            $ele = new XoopsFormPassword($eletitle, $config[$i]->getVar('conf_name'), 50, 255,
                                         $textutil->html_special_chars($config[$i]->getConfValueForOutput()));
            break;
        case 'textbox':
        default:
            $ele = new XoopsFormText($eletitle, $config[$i]->getVar('conf_name'), 50, 255,
                                     $textutil->html_special_chars($config[$i]->getConfValueForOutput()));
            break;
    }
