...
class XooNIpsActionImportUpload extends XooNIpsAction
{
    ...
    /**
     * XooNIpsActionImportUpload constructor.
     */
    ...
    public function preAction()
    {
        ...
        xoonips_validate_request('localfile' === $filetype
                                 || 'remotefile' === $filetype
                                    && $xoopsUser
                                    && $xoopsUser->isAdmin());

        xoonips_validate_request($this->_is_importable_index_id($this->_get_xoonips_checked_index_ids($this->_formdata->getValue('post',
                                                                                                                                 'xoonipsCheckedXID',
                                                                                                                                 's', false))));
    }
    ...
    public function _set_errors_to_import_items($import_items)
    {
        foreach (array_keys($import_items) as $key) {
            if ($import_items[$key]->getDoiConflictFlag()) {
                $import_items[$key]->setErrors(E_XOONIPS_DOI_CONFLICT, 'DOI conflict with following items in existing item.');
            }
        }
    }
