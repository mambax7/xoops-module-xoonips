...
$item = $item_compoHandler->get($item_id);
if (!$item) {
    redirect_header('user.php', 3, _MD_XOONIPS_ITEM_FORBIDDEN);
}
...
if ($op === 'print') {
    ...
    $xoopsTpl->display('db:xoonips_detail_print.tpl');
    xoops_footer();
    exit();
}
...
if ($item_lockHandler->isLocked($item_id)) {
    $xoopsTpl->assign('locked_message',
                      sprintf(_MD_XOONIPS_WARNING_CANNOT_EDIT_LOCKED_ITEM, xoonips_get_lock_type_string($item_lockHandler->getLockType($item_id))));
} else {
    $xoopsTpl->assign('locked_message', false);
}

if ($item_compoHandler->getPerm($item_id, $uid, 'delete') && $op !== 'print') {
...
if ($item_compoHandler->getPerm($item_id, $uid, 'write') && $op !== 'print') {
...
$eventlogHandler->recordViewItemEvent($item_id);
...
$rankingHandler->update();
$viewed_count = $rankingHandler->get_count_viewed_item($item_id);
