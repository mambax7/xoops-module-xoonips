...
if ($item_type_id != ITID_INDEX && !$private_index_flag
    && $index_item_linkHandler->privateIndexReadable($item_id, $xoopsUser->getVar('uid'))
) {
    $op = '';
}
...
if ($op === 'update') {
    ...
    $check_xids = array_unique(array_merge(array($formdata->getValue('post', 'add_to_index_id', 'i', false)),
    ...
    if ($basic) {
        ...
        $xuser               = $xoonips_userHandler->get($basic->get('uid'));
        if (!$private_index_flag && $index_item_linkHandler->privateIndexReadable($item_id, $xoopsUser->getVar('uid'))) {
    ...
    $xoopsTpl->assign('auto_select_private_index_message', $index_item_linkHandler->privateIndexReadable($item_id, $xoopsUser->getVar('uid')));

    $checkedIndexIds        = explode(',', $formdata->getvalue('post', 'xoonipsCheckedXID', 's', false));
    $indexHandler           = xoonips_getOrmHandler('xoonips', 'index');
    $index_item_linkHandler = xoonips_getOrmHandler('xoonips', 'index_item_link');
    $index_item_links       =  $index_item_linkHandler->getObjects(new Criteria('item_id', $formdata->getValue('post', 'item_id', 'i', false)));
    foreach ($index_item_links as $link) {
        if (!$indexHandler->getPerm($link->get('index_id'), $xoopsUser ? $xoopsUser->getVar('uid') : UID_GUEST, 'read')) {
    ...
    if (!isset($change_log) || $change_log == '') {
        ...
        if (count($fields) > 0) {
            $formdata->set('post', 'change_log',
                           sprintf(_MD_XOONIPS_ITEM_CHANGE_LOG_AUTOFILL_TEXT, implode(_MD_XOONIPS_ITEM_CHANGE_LOG_AUTOFILL_DELIMITER, $fields)));
        }
    }
