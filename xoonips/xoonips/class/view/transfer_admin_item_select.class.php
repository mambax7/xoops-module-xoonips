...
class XooNIpsViewTransferAdminItemSelect extends XooNIpsViewTransfer
{
    ...
    public function render()
    {
        ...
        $xoopsTpl->assign(
            'transfer_item_options_hidden', array_merge(
                array_slice($transfer_item_options, 0, ($this->_params['page'] - 1) * 50),
                array_slice($transfer_item_options, ($this->_params['page']) * 50)
            )
        );
        ...
        // breadcrumbs
        $breadcrumbs = [
            ['type' => 'top', 'label' => _AM_XOONIPS_TITLE, 'url' => $xoonips_admin['admin_url'] . '/'],
            ['type' => 'link', 'label' => _AM_XOONIPS_MAINTENANCE_TITLE, 'url' => $xoonips_admin['myfile_url']],
            ['type' => 'link', 'label' => _AM_XOONIPS_MAINTENANCE_ITEM_TITLE, 'url' => $xoonips_admin['mypage_url']],
            ['type' => 'label', 'label' => $title, 'url' => ''],
        ];
    ...
    {
        return ceil(count($this->_params['from_index_item_ids']) / $this->_number_of_item_per_page);
    }
