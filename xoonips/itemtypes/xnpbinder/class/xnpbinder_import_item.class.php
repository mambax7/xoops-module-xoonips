...
{
    /**
     * XNPBinderImportItem constructor.
     */
...
{
    /**
     * XNPBinderImportItemHandler constructor.
     */
    public function __construct()
    {
        parent::__construct();
    ...
    public function onReadFileFinished($item, $import_items)
    {
        ...
        $binderHandler = xoonips_gethandler('xnpbinder', 'import_item');
        if ($binderHandler->publicBinderHasNotPublicItems($child_items, $index_ids)) {
            $item->setErrors(E_XOONIPS_INVALID_VALUE, 'public binder cannot have private and group items');
        }
        if ($binderHandler->groupBinderHasPrivateItems($child_items, $index_ids)) {
        ...
        foreach ($index_ids as $id) {
            ...
            foreach ($child_items as $item) {
                foreach ($item->getImportIndexId() as $child_index_id) {
                    $child_index = $indexHandler->get($child_index_id);
                    if (!$child_index) {
                        continue;
                    }
