...
class XooNIpsAdminFileHandler extends XooNIpsFileHandler
{
    ...
    /**
     * XooNIpsAdminFileHandler constructor.
     */
    public function __construct()
    {
        ...
        $xftHandler = xoonips_getOrmHandler('xoonips', 'file_type');
        // get preview file type id
        $criteria = new Criteria('name', 'preview');
        $xft_objs =  $xftHandler->getObjects($criteria);
        if (count($xft_objs) != 1) {
    ...
    public function getCountFiles()
    {
        ...
        return $this->xfHandler->getCount($criteria);
    }
    ...
    public function getFileIdByCount($num)
    {
        ...
        $xf_objs =  $this->xfHandler->getObjects($criteria);
        if (count($xf_objs) != 1) {
            return false;
        }
    ...
    {
        $xf_obj = $this->xfHandler->get($file_id);
        if (!$xf_obj) {
            return false;
        }
        ...
        $xst_obj = $this->xstHandler->get($file_id);
        if (is_object($xst_obj)) {
            $this->xstHandler->delete($xst_obj);
        }
        ...
        if ($is_deleted || !is_readable($file_path) || null === $fs_name) {
            ...
            return $this->xfHandler->insert($xf_obj);
        }
