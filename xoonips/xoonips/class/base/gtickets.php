        ...
        public function extract_post_recursive($key_name, $tmp_array)
        {
            ...
            foreach ($tmp_array as $key => $val) {
                ...
                if (is_array($val)) {
                    ...
                    $table .= '<tr><th>' . $key_name . '[' . htmlspecialchars($key, ENT_QUOTES) . ']</th><td>' . htmlspecialchars($val, ENT_QUOTES)
                              . '</td></tr>' . "\n";
                    $form .= '<input type="hidden" name="' . $key_name . '[' . htmlspecialchars($key, ENT_QUOTES) . ']" value="'
...
function GTicket_ErrorHandler4FindOutput($errNo, $errStr, $errFile, $errLine)
{
    if (preg_match('?' . preg_quote(XOOPS_ROOT_PATH) . '([^:]+)\:(\d+)?', $errStr, $regs)) {
        echo 'Irregular output! check the file ' . htmlspecialchars($regs[1]) . ' line ' . htmlspecialchars($regs[2], ENT_QUOTES);
    } else {
