...
function xnpsimulatorGetListBlock($item_basic)
{
    ...
    $xnpsimulatorHandler = xoonips_getOrmCompoHandler('xnpsimulator', 'item');
    $tpl->assign('xoonips_item', $xnpsimulatorHandler->getTemplateVar(XOONIPS_TEMPLATE_TYPE_ITEM_LIST, $item_basic['item_id'], $myuid));

    // return as HTML
    return $tpl->fetch('db:xnpsimulator_list_block.tpl');
}
...
function xnpsimulatorGetDetailBlock($item_id)
{
    ...
    $detailHandler = xoonips_getOrmHandler('xnpsimulator', 'item_detail');
    ...
    $tpl->assign('rights', xnpGetRightsDetailBlock($item_id, $detail_orm->getVar('use_cc', 'n'), $detail_orm->getVar('rights', 'n'),
                                                   $detail_orm->getVar('cc_commercial_use', 'n'), $detail_orm->getVar('cc_modification', 'n')));

    $xnpsimulatorHandler = xoonips_getOrmCompoHandler('xnpsimulator', 'item');
    $tpl->assign('xoonips_item', $xnpsimulatorHandler->getTemplateVar(XOONIPS_TEMPLATE_TYPE_ITEM_DETAIL, $item_id, $myuid));

    // return as HTML
    return $tpl->fetch('db:xnpsimulator_detail_block.tpl');
}
...
function xnpsimulatorGetPrinterFriendlyDetailBlock($item_id)
{
    ...
    $tpl->assign('rights', xnpGetRightsPrinterFriendlyBlock($item_id, $detail_orm->getVar('use_cc', 'n'), $detail_orm->getVar('rights', 'n'),
                                                            $detail_orm->getVar('cc_commercial_use', 'n'),
                                                            $detail_orm->getVar('cc_modification', 'n')));
...
function xnpsimulatorGetRegisterBlock()
{
    ...
    $tpl->assign('xnpsimulator_developer',
                 xoonips_get_multiple_field_template_vars(xoonips_get_orm_from_post('xnpsimulator', 'developer'), 'xnpsimulator', 'developer'));

    // return HTML content
    return $tpl->fetch('db:xnpsimulator_register_block.tpl');
}
...
function xnpsimulatorGetEditBlock($item_id)
{
    ...
    $rights = xnpGetRightsEditBlock($item_id, $detail['use_cc']['value'], $detail['rights']['value'], $detail['cc_commercial_use']['value'],
                                    $detail['cc_modification']['value']);
    ...
    $tpl->assign('index', $index);
    $tpl->assign('attachment', $attachment);
    $tpl->assign('attachment_dl_limit',
                 xnpGetDownloadLimitationOptionEditBlock('xnpsimulator', xnpsimulatorGetAttachmentDownloadLimitOption($item_id)));
    $tpl->assign('attachment_dl_notify',
                 xnpGetDownloadNotificationOptionEditBlock('xnpsimulator', xnpsimulatorGetAttachmentDownloadNotifyOption($item_id)));
    $tpl->assign('detail', $detail);
    $tpl->assign('readme', $readme);
    $tpl->assign('rights', $rights);
...
function xnpsimulatorGetConfirmBlock($item_id)
{
    ...
    if (xnpHasWithout($basic) || xnpHasWithout($detail) || xnpHasWithout($preview) || xnpHasWithout($attachment) || xnpHasWithout($readme)
        ...
        || xoonips_is_multiple_field_too_long($developer_objs, 'xnpsimulator', 'developer')
    ) {
    ...
    // return HTML content
    return $tpl->fetch('db:xnpsimulator_confirm_block.tpl');
}
...
function xnpsimulatorInsertItem($item_id)
{
    ...
    $developerHandler = xoonips_getOrmHandler('xnpsimulator', 'developer');
    $developer_objs   = $formdata->getObjectArray('post', $developerHandler->getTableName(), $developerHandler, false);
    if (!$developerHandler->updateAllObjectsByForeignKey('simulator_id', $item_id, $developer_objs)) {
        return false;
    }
    return true;
}
...
function xnpsimulatorUpdateItem($simulator_id)
{
    ...
    $developerHandler = xoonips_getOrmHandler('xnpsimulator', 'developer');
    $developer_objs   = $formdata->getObjectArray('post', $developerHandler->getTableName(), $developerHandler, false);
    if (!$developerHandler->updateAllObjectsByForeignKey('simulator_id', $simulator_id, $developer_objs)) {
        return false;
    }
    return true;
}
...
function xnpsimulatorGetMetaInformation($item_id)
{
    ...
    $xnpsimulatorHandler = xoonips_getOrmCompoHandler('xnpsimulator', 'item');
    $xnpsimulator        = $xnpsimulatorHandler->get($item_id);
...
function xnpsimulatorGetAdvancedSearchBlock($search_var)
{
    ...
    // return HTML content
    return $tpl->fetch('db:xnpsimulator_search_block.tpl');
}
...
function xnpsimulatorGetAdvancedSearchQuery(&$where, &$join)
{
    ...
    $join  = " join $simulator_developer_table on " . $simulator_developer_table . '.simulator_id  = ' . $xoopsDB->prefix('xoonips_item_basic')
             . '.item_id ';
}
...
{
    return xnpGetTopBlock($itemtype['name'], $itemtype['display_name'], 'images/icon_simulator.gif', _MD_XNPSIMULATOR_EXPLANATION,
                          'xnpsimulator_simulator_type', xnpsimulator_get_type_array());
}
...
function xnpsimulatorGetMetadata($prefix, $item_id)
{
    ...
    if ($detail['attachment_dl_limit'] == 0) {
        $files = $fileHandler->getFilesInfo($item_id, 'simulator_data');
    ...
    $previews = $fileHandler->getFilesInfo($item_id, 'preview');
    // rights
