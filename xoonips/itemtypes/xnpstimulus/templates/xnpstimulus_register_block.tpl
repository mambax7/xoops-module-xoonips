...
function onSubmitItemType( form1 ){
  <{$xnpstimulus_developer.table_name}>.trimFields();
  var result = true;
  if( form1.stimulus_data.value == '' && form1.stimulus_dataFileID.value == '' ){
    alert( "<{$smarty.const._MD_XNPSTIMULUS_STIMULUS_FILE_REQUIRED}>" );
    result = false;
  }
  if ( <{$xnpstimulus_developer.table_name}>.getNumOfFields() == 0 ) {
    alert( "<{$smarty.const._MD_XNPSTIMULUS_DEVELOPER_REQUIRED}>" );
    result = false;
  }
  return result;
}
...
<table class="outer">
  ...
  <tr>
    <td class="head"><{$smarty.const._MD_XNPSTIMULUS_DATE_LABEL}></td>
...
<{html_options options=$detail.stimulus_type.select selected=$detail.stimulus_type.value}>
      </select>
    </td>
  </tr>
  <tr>
    <td class="head"><{$smarty.const._MD_XNPSTIMULUS_DEVELOPER_LABEL}><{$smarty.const._MD_XOONIPS_ITEM_REQUIRED_MARK}></td>
    <td class="<{cycle name="oddeven"}>">
<{include file='db:xoonips_register_multiple_field.tpl' vars=$xnpstimulus_developer}>
    </td>
  </tr>
  <tr>
  ...
  <tr>
    <td class="head"><{$smarty.const._MD_XNPSTIMULUS_STIMULUS_FILE}><{$smarty.const._MD_XOONIPS_ITEM_REQUIRED_MARK}></td>
...
<!-- end of xnpstimulus_register_block.tpl -->
