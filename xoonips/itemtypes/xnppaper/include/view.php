...
function _xnppaper_get_detail_information_objs($item_id)
{
    ...
    $ret['detail']  = array();
    $ret['authors'] = array();
    $detailHandler  = xoonips_getOrmHandler('xnppaper', 'item_detail');
    $ret['detail']  = $detailHandler->get($item_id);
...
{
    $pubmed_id = trim((string)$pmid);
    if (preg_match('/^(\\d+)$/', $pubmed_id)) {
        $pubmed_link = '<a href="http://www.ncbi.nlm.nih.gov/entrez/query.fcgi?cmd=Retrieve&amp;db=pubmed&amp;list_uids=' . $pubmed_id
                       . '" target="_blank">' . $pubmed_id . '</a>';
    } else {
...
function xnppaperGetMetaInformation($item_id)
{
    ...
    $xnppaperHandler = xoonips_getOrmCompoHandler('xnppaper', 'item');
    $xnppaper        = $xnppaperHandler->get($item_id);
...
function xnppaperGetListBlock($item_basic)
{
    ...
    $xnppaperHandler = xoonips_getOrmCompoHandler('xnppaper', 'item');
    $tpl->assign('xoonips_item', $xnppaperHandler->getTemplateVar(XOONIPS_TEMPLATE_TYPE_ITEM_LIST, $item_basic['item_id'], $myuid));

    // return as HTML
    return $tpl->fetch('db:xnppaper_list_block.tpl');
}
...
function xnppaperGetDetailBlock($item_id)
{
    ...
    $xnppaperHandler = xoonips_getOrmCompoHandler('xnppaper', 'item');
    $tpl->assign('xoonips_item', $xnppaperHandler->getTemplateVar(XOONIPS_TEMPLATE_TYPE_ITEM_DETAIL, $item_id, $myuid));

    // return as HTML
    return $tpl->fetch('db:xnppaper_detail_block.tpl');
}
...
function xnppaperGetRegisterBlock()
{
    ...
    // return HTML
    return $tpl->fetch('db:xnppaper_register_block.tpl');
}
...
function xnppaperGetEditBlock($item_id)
{
    ...
    if (!$formdata->getValue('get', 'post_id', 's', false)) {
        ...
        $tpl->assign('xnppaper_author',
                     xoonips_get_multiple_field_template_vars(xoonips_get_orm_from_post('xnppaper', 'author'), 'xnppaper', 'author'));
    }
...
function xnppaperGetConfirmBlock($item_id)
{
    $textutil      = xoonips_getUtility('text');
    $formdata      = xoonips_getUtility('formdata');
    $authorHandler = xoonips_getOrmHandler('xnppaper', 'author');
    ...
    if (xnpHasWithout($basic) || xnpHasWithout($paper_pdf_reprint) || xnpHasWithout($detail)
        || xoonips_is_multiple_field_too_long($author_objs, 'xnppaper', 'author')
    ) {
    ...
    // return HTML
    return $tpl->fetch('db:xnppaper_confirm_block.tpl');
}
...
function xnppaperInsertItem($item_id)
{
    ...
    $vals = implode(',', array(
        '\'' . addslashes($ar['journal']) . '\'',
        strlen($volume) == 0 ? 'null' : (int)$volume,
        strlen($number) == 0 ? 'null' : (int)$number,
        strlen($page) == 0 ? 'null' : '\'' . addslashes($ar['page']) . '\'',
        strlen($abstract) == 0 ? 'null' : '\'' . addslashes($ar['abstract']) . '\'',
        strlen($pubmed_id) == 0 ? 'null' : '\'' . addslashes($ar['pubmed_id']) . '\''
    ));
    ...
    $authorHandler = xoonips_getOrmHandler('xnppaper', 'author');
    $author_objs   = $formdata->getObjectArray('post', $authorHandler->getTableName(), $authorHandler, false);
    if (!$authorHandler->updateAllObjectsByForeignKey('paper_id', $item_id, $author_objs)) {
        return false;
    }
    return true;
}
...
function xnppaperUpdateItem($item_id)
{
    ...
    // register detail information
    $sql    = implode(',', array(
        'journal' . '=\'' . addslashes($ar['journal']) . '\'',
        'volume' . '=' . (strlen($volume) == 0 ? 'null' : (int)$volume),
        'number' . '=' . (strlen($number) == 0 ? 'null' : (int)$number),
        'page' . '=' . (strlen($page) == 0 ? 'null' : '\'' . addslashes($ar['page']) . '\''),
        'abstract' . '=' . (strlen($abstract) == 0 ? 'null' : '\'' . addslashes($ar['abstract']) . '\''),
        'pubmed_id' . '=' . (strlen($pubmed_id) == 0 ? 'null' : '\'' . addslashes($ar['pubmed_id']) . '\'')
    ));
...
function xnppaperGetDetailInformationQuickSearchQuery(&$wheres, &$join, $keywords)
{
    ...
    $wheres = xnpGetKeywordsQueries(array(
                                        "$paper_table.journal",
                                        "$paper_table.pubmed_id",
                                        "$paper_author_table.author"
                                    ), $keywords);
    return true;
}
...
function xnppaperGetAdvancedSearchBlock($search_var)
{
    ...
    // return HTML
    return $tpl->fetch('db:xnppaper_search_block.tpl');
}
...
function xnppaperExportItem($export_path, $fhdl, $item_id, $attachment)
{
    ...
    if (!fwrite($fhdl,
                "<detail id=\"${item_id}\" version=\"1.02\">\n" . "<authors>{$authors}</authors>\n" . '<journal>' . $detail->getVar('journal', 's')
                . "</journal>\n" . '<volume>' . $detail->getVar('volume', 's') . "</volume>\n" . '<number>' . $detail->getVar('number', 's')
                . "</number>\n" . '<page>' . $detail->getVar('page', 's') . "</page>\n" . '<abstract>' . $detail->getVar('abstract', 's')
...
function xnppaperGetModifiedFields($item_id)
{
    ...
    if ($detail) {
        foreach (array(
                     ...
                     'pubmed_id' => _MD_XNPPAPER_PUBMED_ID_LABEL
                 ) as $k => $v
        ) {
...
function xnppaperGetMetadata($prefix, $item_id)
{
    ...
    if (!empty($detail['page'])) {
        if (preg_match('/^(\d+)\s*[- ,_]+\s*(\d+)$/', $detail['page'], $matches)) {
            $detail['start_page'] = (int)$matches[1];
            $slen                 = strlen($matches[1]);
            $elen                 = strlen($matches[2]);
            if ($slen <= $elen) {
                $detail['end_page'] = (int)$matches[2];
            } else {
                $detail['end_page'] = (int)(substr($matches[1], 0, $slen - $elen) . $matches[2]);
            }
        } else {
            $detail['start_page'] = $detail['end_page'] = (int)$detail['page'];
        }
    }
    ...
    if ($mconfig['pdf_access_rights'] == 1) {
        $files = $fileHandler->getFilesInfo($item_id, 'paper_pdf_reprint');
