...
class XNPFilesImportItem extends XooNIpsImportItem
{
    ...
    /**
     * XNPFilesImportItem constructor.
     */
...
/**
 * Class XNPFilesImportItemHandler.
 */
class XNPFilesImportItemHandler extends XooNIpsImportItemHandler
{
    ...
    /**
     * XNPFilesImportItemHandler constructor.
     */
    public function __construct()
    {
        parent::__construct();
    ...
    public function xmlStartElementHandler($parser, $name, $attribs)
    {
        ...
        switch (implode('/', $this->_tag_stack)) {
            ...
            case 'ITEM/DETAIL/FILE':
                ...
                $this->_files_file = $fileHandler->create();
                $this->_files_file->setFilepath($this->_attachment_dir.'/'.$attribs['FILE_NAME']);
                $this->_files_file->set('original_file_name',
    ...
    public function xmlEndElementHandler($parser, $name)
    {
        ...
        switch (implode('/', $this->_tag_stack)) {
            ...
            case 'ITEM/DETAIL/FILE':
                ...
                $fileHandler = xoonips_getOrmHandler('xoonips', 'file');
                if (!$fileHandler->insert($this->_files_file)) {
                    $this->_import_item->setErrors(E_XOONIPS_DB_QUERY, "can't insert attachment file:".$this->_files_file->get('original_file_name')
                                                                       .$this->_get_parser_error_at());
                }
                $this->_files_file = $fileHandler->get($this->_files_file->get('file_id'));
    ...
    public function import($item)
    {
        if ($item->getUpdateFlag()) {
            ...
            if ($item->hasFilesFile()) {
                $fileHandler = xoonips_getOrmHandler('xoonips', 'file');
                $clonefile = $fileHandler->fileClone($files_file);
                $clonefile->setDirty();
