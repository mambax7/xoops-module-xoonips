...
function xnpGetBasicInformationArray($item_id, $fmt = 'n')
{
    ...
    $itemlib_obj    = $itemlibHandler->get($item_id);
...
{
    $itemlibHandler = XooNIpsItemLibraryHandler::getInstance();
    $itemlib_obj    = $itemlibHandler->create();
    $itemlibHandler->fetchRequest($itemlib_obj, true);
...
function xnpGetBasicInformationConfirmBlock($item_id)
{
    ...
    $itemlibHandler->fetchRequest($itemlib_obj, false);
...
function xnpInsertBasicInformation(&$item_id)
{
    ...
    $itemlib_obj    = $itemlibHandler->create();
    $itemlibHandler->fetchRequest($itemlib_obj, false);
    if (!$itemlibHandler->insertBasicInformation($itemlib_obj)) {
        return false;
    }
...
function xnpUpdateBasicInformation($item_id)
{
    ...
    $itemlib_obj    = $itemlibHandler->get($item_id);
    $itemlibHandler->fetchRequest($itemlib_obj, false);
    return $itemlibHandler->insertBasicInformation($itemlib_obj);
}
...
function xnpGetItemTypeDisplayNameByDirname($dirname, $fmt)
{
    ...
    $item_type_objs   =  $item_typeHandler->getObjects($criteria);
...
function xnpCleanup()
{
    ...
    $sql
            = "select file_id from $fileTable as tf left join $sessionTable as ts on tf.sess_id=ts.sess_id where tf.item_id is NULL and ts.sess_id is NULL";
...
function xnpGetUploadDir()
{
    ...
    $uploadDir      = $xconfigHandler->getValue('upload_dir');
    if (empty($uploadDir)) {
...
function xnpGetFileInfo($columns, $condition, $item_id)
{
    ...
    $sql    = "select $columns from " . $xoopsDB->prefix('xoonips_file') . ' as t_file, ' . $xoopsDB->prefix('xoonips_file_type') . ' as t_file_type '
              . " where t_file.file_type_id = t_file_type.file_type_id and $condition and $condition2 ";
    $result = $xoopsDB->query($sql);
    if ($result == false) {
        echo "xnpGetFileInfo: $sql " . $xoopsDB->error();
        return false;
    }
...
function xnpGetPreviewDetailBlock($item_id)
{
    ...
    while (list($dummy, list($fileID, $caption)) = each($files)) {
        ...
        $imageHtml1[]      = '<a href="' . $imageFileName . '" target="_blank"><img src="' . $thumbnailFileName . '" alt="' . $htmlCaption
                             . '"/></a>';
...
function xnpGetAttachmentDetailBlock($item_id, $name)
{
    ...
    $files
          = xnpGetFileInfo('t_file.file_id, t_file.original_file_name, t_file.file_size, t_file.mime_type, unix_timestamp(t_file.timestamp), download_count',
                           "t_file_type.name='$name' and sess_id is NULL and is_deleted=0", $item_id);
    if ($files == false || count($files) == 0 || $res != RES_OK) {
        ...
        $warning            = '';
        $button_img         = '<img src="images/icon_button.php?label=download&amp;mode=normal" alt="' . _MD_XOONIPS_ITEM_DOWNLOAD_LABEL
                              . '" id="xoonips_download_button_' . $fileID . '"/>';
        ...
        if (function_exists($func) && $func($item_id, $name)) {
            $download_button
                     = "<a $button_href $button_onfocus $button_onblur $button_onmousedown $button_onmouseup $button_onmouseover $button_onmouseout $button_onkeypress $button_onclick>$button_img</a>";
            $warning = '<noscript><span style="color: red;">' . _MD_XOONIPS_ITEM_DOWNLOAD_NOSCRIPT_LABEL . '</span><br></noscript>';
        } else {
            $download_button
                = "<a $button_href $button_onfocus $button_onblur $button_onmousedown $button_onmouseup $button_onmouseover $button_onmouseout $button_onkeypress>$button_img</a>";
        }
 ...
 * @param item_id $item_id
 * @param if      $download_file_id
 * @param         download_file_id     if non-false value, automatically push download button of this file_id
 * @param use     $use_license
 * @param use     $use_cc
 * @param license $rights
 * @return mixed|string|void
 * @internal param item_id $item_id
 */
function xnpGetDownloadConfirmationBlock($item_id, $download_file_id, $attachment_dl_notify, $use_license, $use_cc, $rights)
{
    ...
    return $tpl->fetch('db:xoonips_detail_download_confirmation.tpl');
}
...
function xoonips_get_download_filename($file_id)
{
    ...
    if ($download_file_compression === 'on') {
        ...
        return mb_decode_numericentity($unicode->encode_utf8($file->get('original_file_name'), xoonips_get_server_charset()),
 ...
 * @return array
 * @internal param item_id $item_id
 * @internal param name $name of file type
 */
        ...
        foreach ($groupby as $open_level => $i) {
            foreach ($i as $owner_gid => $j) {
                foreach ($j as $certified => $k) {
                    ...
                    $buttons = '';
                    if ($button_flag) {
                        if ($index_item_linkHandler->getPerm($xid, $item_id, $uid, 'withdraw')) {
                            $buttons .= '<input class="formButton" type="button" value="' . _MD_XOONIPS_ITEM_WITHDRAW_BUTTON_LABEL
                                        . '" onclick="xoonips_certify_confirm( [' . implode(',', $xid_array) . '], ' . $item_id
                                        . ', \'withdraw\');"/>';
                        }
                        if ($index_item_linkHandler->getPerm($xid, $item_id, $uid, 'accept')) {
                            $buttons .= '<input class="formButton" type="button" value="' . _MD_XOONIPS_ITEM_CERTIFY_BUTTON_LABEL
                                        . '" onclick="xoonips_certify_confirm( [' . implode(',', $xid_array) . '], ' . $item_id
                                        . ', \'accept_certify\');"/>';
                        }
                        if ($index_item_linkHandler->getPerm($xid, $item_id, $uid, 'reject')) {
                            $buttons .= '<input class="formButton" type="button" value="' . _MD_XOONIPS_ITEM_UNCERTIFY_BUTTON_LABEL
                                        . '" onclick="xoonips_certify_confirm( [' . implode(',', $xid_array) . '], ' . $item_id
                                        . ', \'reject_certify\');"/>';
                        }
                    }
...
function xnpUploadFile($name, $keyval)
{
    ...
    if ($result == false) {
        return array(
            false,
            "xnpUploadFile: bad sql $sql " . $xoopsDB->error()
        );
    }
    ...
    if (empty($fileTypeID)) {
        return array(
            false,
            "xnpUploadFile: no filetype '$name' "
        );
    }
    ...
    if (!empty($uploadTmpDirPath)) {
        if (!is_writable($uploadTmpDirPath)) {
            return array(
                false,
                'xnpUploadFile: cannot write. in ' . $uploadTmpDirPath
            );
        }
    }
    if (!is_writable(xnpGetUploadDir())) {
        return array(
            false,
            'xnpUploadFile: cannot write. in ' . xnpGetUploadDir()
        );
    } else {
        ...
        if ($result == false) {
            return array(
                false,
                "xnpUploadFile: cannot move_uploaded_file. \n $name " . $file['tmp_name'] . ' to ' . $filePath
            );
        }
    }
    ...
    $admin_fileHandler->updateFileSearchText($fileID, true);
...
function xnpGetPreviewEditBlock($item_id)
{
    ...
    $uploadHtml .= '<input class="formButton" type="button" name="preview_upload_button_' . $fileID . '" value="' . _MD_XOONIPS_ITEM_UPLOAD_LABEL
                   . '" onclick="xnpSubmitFileUpload(this.form, \'preview\')"/></td>';
    $uploadHtml .= '</tr>';
    $uploadHtml .= '<tr>';
    ...
    while (list($dummy, list($fileID, $caption)) = each($files)) {
        ...
        $imageHtml2[]      = "$htmlCaption";
        $imageHtml3[]      = '<input class="formButton" type="button" name="preview_delete_button_' . $fileID . '" value="'
                             . _MD_XOONIPS_ITEM_DELETE_BUTTON_LABEL . '" onclick="xnpSubmitFileDelete( this.form, \'preview\', ' . $fileID . ' )"/>';
    }
    $html = xnpMakeTable(array(
                             $imageHtml1,
                             $imageHtml2,
...
function xnpGetAttachmentEditBlock($item_id, $name)
{
    ...
    if (empty($fileInfo)) {
        ...
        $fileInfoLine = "$fileName  ($fileSize Bytes) <input class='formButton' type='button' name='file_delete_button_" . $fileID . "' value='"
                        . _MD_XOONIPS_ITEM_DELETE_BUTTON_LABEL . "' onclick=\"xnpSubmitFileDelete( this.form, '$name', $fileID )\" />";
    }
    $html
        = "
...
/**
 *
 * @param module   $dirname
 * @param download $option
 * @return array
 * @internal param module $dirname dirname
 * @internal param download $option limit option(0:everyone, 1:login user only)
 */
...
function xnpGetDownloadLimitationOptionRegisterBlock($dirname, $option = 0)
{
    ...
    if ($enable_dl_limit == 1) {
        ...
        $html .= ' />' . _MD_XOONIPS_ITEM_ATTACHMENT_DL_LIMIT_LOGINUSER_LABEL . "<input type='radio' name='attachment_dl_limit' value='0'";
        if ($option != 1) {
            $html .= ' checked';
        }
        ...
        $html = _MD_XOONIPS_ITEM_ATTACHMENT_DL_LIMIT_EVERYONE_LABEL . "<input type='hidden' name='attachment_dl_limit' value='0'/>";
    }
    return array(
        'name'  => _MD_XOONIPS_DOWNLOAD_LIMITATION_OPTION_LABEL,
        'value' => $html
    );
}
...
function xnpGetDownloadLimitationOptionConfirmBlock($dirname)
{
    ...
    if ($enable_dl_limit == 1) {
        ...
        if (isset($attachment_dl_limit) && $attachment_dl_limit == 1) {
            $html .= _MD_XOONIPS_ITEM_ATTACHMENT_DL_LIMIT_LOGINUSER_LABEL . "<input type='hidden' name='attachment_dl_limit' value='1'/>";
        } else {
            $html .= _MD_XOONIPS_ITEM_ATTACHMENT_DL_LIMIT_EVERYONE_LABEL . "<input type='hidden' name='attachment_dl_limit' value='0'/>";
        }
    } else {
...
function xnpGetDownloadNotificationOptionRegisterBlock($dirname, $option = 0)
{
    ...
    if (isset($assoc['enable_dl_limit']) && $assoc['enable_dl_limit'] == '1') {
        ...
        $html .= ' />' . _MD_XOONIPS_ITEM_ATTACHMENT_DL_NOTIFY_DO_LABEL . "<input type='radio' name='attachment_dl_notify' value='0'";
        if ($option != 1) {
            $html .= ' checked';
        }
        $html .= ' />' . _MD_XOONIPS_ITEM_ATTACHMENT_DL_NOTIFY_DONT_LABEL . '<br> ' . _MD_XOONIPS_ITEM_ATTACHMENT_DL_NOTIFY_EXPLANATION;
    } else {
        $html = _MD_XOONIPS_ITEM_ATTACHMENT_DL_NOTIFY_DONT_LABEL . "<input type='hidden' name='attachment_dl_notify' value='0'/>";
    }
    return array(
        'name'  => _MD_XOONIPS_DOWNLOAD_NOTIFICATION_OPTION_LABEL,
        'value' => $html
    );
}
...
function xnpGetDownloadNotificationOptionConfirmBlock($dirname)
{
    ...
    if (isset($assoc['enable_dl_limit']) && $assoc['enable_dl_limit'] == '1') {
        ...
        if (isset($attachment_dl_notify) && $attachment_dl_notify == 1) {
            $html = _MD_XOONIPS_ITEM_ATTACHMENT_DL_NOTIFY_DO_LABEL . "<input type='hidden' name='attachment_dl_notify' value='1'/>";
        } else {
            $html = _MD_XOONIPS_ITEM_ATTACHMENT_DL_NOTIFY_DONT_LABEL . "<input type='hidden' name='attachment_dl_notify' value='0'/>";
        }
    } else {
...
function xnpGetTextFileEditBlock($item_id, $name, $defaultText)
{
    ...
    <td style='vertical-align: text-bottom; text-align:right;'><a href='#' onclick=\"return xnpOpenTextFileInputWindow('$name',$item_id)\">"
          . _MD_XOONIPS_ITEM_TEXT_FILE_EDIT_LABEL . "</a></td>\n
    </tr></table>\n
...
function xnpGetTextFilePrinterFriendlyBlock($item_id, $name, $text)
{
    ...
    return array(
        'name'  => _MD_XOONIPS_ITEM_TEXTFILE_LABEL,
        'value' => $myts->nl2Br($textutil->html_special_chars($text))
    );
}
...
function xnpGetPreviewConfirmBlock($item_id)
{
    ...
    if (empty($previewFileID)) {
        ...
        $html = xnpMakeTable(array(
                                 $imageHtml1,
                                 $imageHtml2
                             ), 3) . "<input type='hidden' name='previewFileID' value='$previewFileID' />";
    }
...
function xnpGetAttachmentConfirmBlock($item_id, $name)
{
    ...
    if (!empty($_FILES[$name]['name'])) {
        ...
        if ($fileID == false) {
            ...
            $system_message = $system_message . "\n" . '<br><span style="color: red;">' . $textutil->html_special_chars($errorMessage)
                              . '</span><br>';
            return false;
        } else {
    ...
    $files = xnpGetFileInfo('t_file.file_id, t_file.original_file_name, t_file.file_size, t_file.mime_type, unix_timestamp(t_file.timestamp)',
                            "t_file_type.name='$name' and is_deleted = 0 and $sql ", $item_id);
...
function xnpGetIndexConfirmBlock($item_id)
{
    ...
    foreach ($index_ids as $xid) {
        if (!$indexHandler->getPerm($xid, $_SESSION['xoopsUserId'], 'read')) {
            continue;
        }
        $str           = xnpGetIndexPathString($xnpsid, $xid);
        $indexes[$xid] = "$str";
    ...
    return array(
        'name'  => _MD_XOONIPS_ITEM_INDEX_LABEL,
        'value' => implode('<br>', array_values($indexes))
    );
}
...
function xnpGetIndexRegisterBlock()
{
    ...
    if (count($indexes) == 0) {
        ...
        return array(
            'name'  => _MD_XOONIPS_ITEM_INDEX_LABEL . _MD_XOONIPS_ITEM_REQUIRED_MARK,
            'value' => implode('<br>', array_values($indexes))
        );
    }
}
...
function xoonips_insert_event_and_send_notification_of_certification($item_id)
{
    ...
    foreach ($index_ids as $i) {
        ...
        if ($result == RES_OK) {
            ...
            $eventlogHandler->recordRequestCertifyItemEvent($item_id, $i);
            if ($certify_item === 'auto') {
                xoonips_notification_item_certified_auto($item_id, $i);

                // record events(certify item)
                $eventlogHandler->recordCertifyItemEvent($item_id, $i);
...
function xnpUpdatePreview($item_id)
{
    ...
    if (empty($previewFileID)) {
        ...
        $sql
                = "update $table set sess_id='$esc_sess_id', item_id=NULL where item_id=$item_id and file_id not in ($previewFileID) and file_type_id=$file_type_id";
        $result = $xoopsDB->queryF($sql);
        if ($result != false) {
            $sql
                = "update $table set sess_id=NULL, item_id=$item_id where sess_id='$esc_sess_id' and file_id in ($previewFileID) and file_type_id=$file_type_id";
            $result &= $xoopsDB->queryF($sql);
        }
    }
    if ($result == false) {
        echo "Error: cannot update xoonips_file $sql " . $xoopsDB->error();
        return false;
    }
 ...
 * @internal param readme $name /rights
 */
...
function xnpGetBasicInformationAdvancedSearchBlock($moduleName, $search_var)
{
    ...
    return array(
        'title'             => array(
            ...
            'value' => '<input type="text" name="' . $moduleName . '_title" value="" size="50"/>'
        ),
        'keywords'          => array(
            'name'  => _MD_XOONIPS_ITEM_KEYWORDS_LABEL,
            'value' => '<input type="text" name="' . $moduleName . '_keywords" value="" size="50"/>'
        ),
        'description'       => array(
            'name'  => _MD_XOONIPS_ITEM_DESCRIPTION_LABEL,
            'value' => '<input type="text" name="' . $moduleName . '_description" value="" size="50"/>'
        ),
        'doi'               => array(
            'name'  => _MD_XOONIPS_ITEM_DOI_LABEL,
            'value' => '<input type="text" name="' . $moduleName . '_doi" value="" size="50"/>'
        ),
        'publication_date'  => array(
            'name'  => _MD_XOONIPS_ITEM_PUBLICATION_DATE_LABEL,
            'value' => $tpl->fetch('db:xoonips_search_date.tpl')
        ),
        'publication_year'  => array(
            'name'  => _MD_XOONIPS_ITEM_PUBLICATION_YEAR_LABEL,
            'value' => $tpl->fetch('db:xoonips_search_year.tpl')
        ),
        'publication_month' => array(
            'name'  => _MD_XOONIPS_ITEM_PUBLICATION_MONTH_LABEL,
            'value' => $tpl->fetch('db:xoonips_search_month.tpl')
        ),
        'publication_mday'  => array(
            'name'  => _MD_XOONIPS_ITEM_PUBLICATION_MDAY_LABEL,
            'value' => $tpl->fetch('db:xoonips_search_mday.tpl')
        )
    );
}
 ...
 * @return array|result
 * @internal param Input $keywords keyword
 */
 ...
 * @internal param input $keyword keyword
 */
        ...
        if ($str === '(') {
            ...
            if ($expectTerm || $nest < 0) {
                return array(
                    array(),
                    array(),
                    _MD_XOONIPS_ITEM_SEARCH_SYNTAX_ERROR
                );
            }
            $expectTerm = false;
            $elements[] = $lowerstr;
 ...
 * @param character $wheres
 * @return string
 * @internal param input $elements keyword is resolved with xnpSplitKeywords2.
 * @internal param character $wheres strings in retrieval keyword is converted into SQL sentense.
 */
 ...
 * @param table $dbVarName
 * @param Name  $postVarName
 * @return string
 * @internal param table $dbVarName name, and column name in database
 * @internal param Name $postVarName of variables posted
 */
function xnpGetKeywordQuery($dbVarName, $postVarName)
{
    ...
    foreach ($keywords as $keyword) {
        $escKeyword = addslashes(str_replace(array(
                                                 '_',
                                                 '%',
                                                 "\\"
                                             ), array(
 ...
 * @param array $dbVarNames
 * @param array $keywords
 * @return array of query ([n]: $keywords[n] is contained in one column in array of $dbVarNames at least.)
 * @internal param array $dbVarNames of table name and column name in database.
 * @internal param array $keywords of keywords
 */
 ...
 * @return query|string
 * @internal param name $moduleName of module
 */
 ...
 * @param array $iids
 * @return float|sum
 * @internal param array $iids of item_id
 */
 ...
 * @internal param ID $item_id of retrieval item
 */
function xnpIsPending($item_id)
{
    $index_linkHandler = xoonips_getOrmHandler('xoonips', 'index_item_link');
    ...
    $cnt = $index_linkHandler->getCount($criteria, $join);
    return ($cnt != 0);
}
 ...
 * @param how       $search_itemtype
 * @param true      $private_flag
 * @param reference $msg
 * @param reference $iids
 * @param           $search_var
 * @param search    $search_cache_id
 * @param           $search_tab
 * @param string    $file_or_item_metadata
 * @return array|true
 * @internal param $op 'quicksearch' 'advancedsearch' 'itemsubtypesearch' 'itemtypesearch'
 * @internal param search $keyword keyword
 * @internal param how $search_itemtype to search ('all', 'basic' or name of itemtype (ex.xnppaper) )
 * @internal param true $private_flag if search private indexes.
 * @internal param reference $msg to variables that receive  error message
 * @internal param reference $iids to array that receive item id that match query condition
 * @internal param search $search_cache_id cache id(in/out)
 * @internal param $search_tab 'item'/'metadata'/'file' (it regards illegal value as 'item')
 * @internal param $file_or_item_metadata 'file'=search_text table only, 'item_metadata'=other than search_text table, 'all'=all. effective only if op==quicksearch && search_itemtype!=basic
 */
function xnpSearchExec($op, $keyword, $search_itemtype, $private_flag, $msg, $iids, $search_var, $search_cache_id, $search_tab,
    ...
    if ($search_cache_id) {
        ...
        $sql            = "select count(*) from $event_log_table where event_type_id in (" . implode(',', $event_type_ids)
        ...
        if ($count == 0) {
            if ($search_tab === 'metadata') {
                ...
                $sql
                    = "select tf.item_id    from $cache_file_table as tcf
                ...
                $sql
                    = "select tci.item_id    from $cache_item_table as tci
    ...
    if ($private_flag) { // operation to add item into index. search for only the user's item.
        ...
        if ($xmemberHandler->isAdmin($uid) || xnp_is_moderator($xnpsid, $uid)) {
            $privilege = ' 1 ';
        } else {
            $privilege = " ($index_table.open_level = " . OL_PUBLIC . " or \n" . "  $index_table.open_level = " . OL_PRIVATE
                         . " and $index_table.uid=$uid or \n" . "  $index_table.open_level = " . OL_GROUP_ONLY . " and $glink_table.uid=$uid ) \n";
        }
    }
    ...
    if ($op === 'advancedsearch' || $op === 'itemsubtypesearch') {
        ...
        foreach ($itemtypes as $itemtype_id => $itemtype) {
            ...
            if ($formdata->getValue('post', $module_name, 'n', false)) {
                ...
                if ($where != '') {
                    $sql = "select $basic_table.item_id, $search_cache_id from $basic_table " . $join1
                           ...
                           . " left join $search_text_table on $search_text_table.file_id    = $file_table.file_id " . $join
        ...
        if ($search_itemtype == 'basic') { // search titles and keywords
            $wheres_title_keyword = xnpGetKeywordsQueries(array(
                                                              $title_table . '.title',
        ...
        if ($search_itemtype == 'metadata' || $search_itemtype == 'all') {
            ...
            $sql
                = "select identifier, $search_cache_id
        ...
        if (isset($itemtype_names[$search_itemtype]) || $search_itemtype == 'all') {
            ...
            */
            ...
            $wheres_basic = xnpGetKeywordsQueries(array(
                                                      $title_table . '.title',
                                                      $keyword_table . '.keyword',
                ...
                if ($search_itemtype == $module_name || $search_itemtype == 'all') {
                    ...
                    if ($file_or_item_metadata === 'all' || $file_or_item_metadata === 'item_metadata') {
                        ...
                        $sql      = "select $basic_table.item_id, $search_cache_id from $basic_table " . $join1
                                    ...
                                    . " left join $title_table   on $basic_table.item_id = $title_table.item_id "
                                    . " left join $keyword_table on $basic_table.item_id = $keyword_table.item_id "
                                    . " left join $table on $key_name = $basic_table.item_id " . $join . " where $where and $privilege \n"
                                    . " group by $basic_table.item_id  \n";
                    ...
                    if ($file_or_item_metadata === 'all' || $file_or_item_metadata === 'file') {
                        ...
                        // search inside files
                        $sql
                            = "insert ignore into $cache_file_table ( file_id, search_cache_id )
 ...
 * @param true             $is_absolute
 * @param bool|is_absolute $base_index_id
 * @return true :success, false:failure
 * @internal param file $fhdl handle writes outputs
 * @internal param true $is_absolute :index tags are absolute path. false: index tags are relative path to base_index_id.
 * @internal param is_absolute $base_index_id == false && base_index_id == false: outputs only 1 empty index tag "<index></index>"
...
function xnpBasicInformation2XML($fhdl, $item, $is_absolute, $base_index_id = false)
{
    ...
    if (!fwrite($fhdl, "<basic id=\"${item['item_id']}\">\n" . "<itemtype>${itemtype}</itemtype>\n" . '<titles>' . $titles . "</titles>\n"
                       . "<contributor uname='" . $myts->htmlSpecialChars($account['uname']) . "'>" . $myts->htmlSpecialChars($contributor)
                       . "</contributor>\n" . '<keywords>' . $keywords . "</keywords>\n" . '<description>'
                       . $myts->htmlSpecialChars($item['description']) . "</description>\n" . '<doi>' . $myts->htmlSpecialChars($item['doi'])
                       ...
                       . "/modules/xoonips/detail.php?item_id=${item['item_id']}</url>\n")
    ) {
        return false;
    }
    ...
    if ($base_index_id) {
        foreach ($index_id as $i) {
            ...
            if (!fwrite($fhdl,
                        "<index open_level='" . $open_level_str[$index['open_level']] . "'>" . $myts->htmlSpecialChars($head . $str) . "</index>\n")
            ) {
                return false;
            }
        }
...
function xnpGetTopBlock($moduleName, $displayName, $iconPath, $explanation, $subtypeVarName, $subtypes)
{
    ...
    // Output in HTML.
    return $tpl->fetch('db:xoonips_top_itemtype_block.tpl');
}
...
function xnpGetModifiedFields($item_id)
{
    ...
    if (xnp_get_item($xnpsid, $item_id, $item) == RES_OK) {
        foreach (array(
                     'contributor'      => _MD_XOONIPS_ITEM_CONTRIBUTOR_LABEL,
...
function xnpGetBasicInformationMetadata($metadataPrefix, $item_id)
{
    ...
    if ($metadataPrefix === 'junii' || $metadataPrefix === 'junii2') {
        ...
        $lines[] = "<identifier xsi:type=\"URL\">" . $textutil->xml_special_chars(xnpGetItemDetailURL($basic['item_id'], $basic['doi']))
                   . '</identifier>';
 ...
 * @param               item_id item_id
 * @param int           $use_cc
 * @param Rights|string $text
 * @param int           $cc_commercial_use
 * @param int           $cc_modification
 * @return array
 * @internal param Rights $text text or html
 */
function xnpGetRightsDetailBlock($item_id, $use_cc = 1, $text = '', $cc_commercial_use = 1, $cc_modification = 2)
{
    $textutil = xoonips_getUtility('text');
    $hidden   = xnpCreateHidden('rightsUseCC', $use_cc) . xnpCreateHidden('rightsEncText', $text) . xnpCreateHidden('rightsCCCommercialUse',
                                                                                                                    $cc_commercial_use)
    ...
    if ($use_cc) {
        ...
        return array(
            'name'   => _MD_XOONIPS_ITEM_TEXTFILE_LABEL,
            'value'  => '<textarea readonly="readonly" rows="5" cols="40" style="width:320px;">' . $textutil->html_special_chars($text)
                        . '</textarea>',
            'hidden' => $hidden
        );
    }
 ...
 * windowに重なる部分の文字数 < window_size && windowに重ならない部分の文字数 > 0 なら そのときのbin2hex()はleadingかtrailing
 *
 * 例: window_size=3とすると
 * +-leading-+---------middle---------+-trailing-+
 * あいうえお →   あ  あい   あいう  いうえ  うえお  えお  お
 * あい       →   あ         あい                    い
 *
 * output_leading: leadingを出力するならtrue
 * output_trailing: trailingを出力するならtrue
 * @param $str
 * @param $output_leading
 * @param $output_trailing
 * @return array
 */
...
function xnpWordSeparation($str, $output_leading = true, $output_trailing = true)
{
    ...
    while ($w1 < $end) {
        if ($multibyte_mode) {
            ...
            $ar = xnpWindowString(mb_substr($str, $w0, $w1 - $w0, $encoding), $w0 != 0 || $output_leading, $w1 != $end || $output_trailing);
        } else {
...
function xnpGetColumnLengths($table_wo_prefix)
{
    ...
    $ar = array();
    for ($i = 0, $iMax = mysqli_num_fields($result); $i < $iMax; $i++) {
 ...
 * @param return $iids
 * @return int|RES_OK
 * @internal param return $iids item id of each doi(array)
 */
 ...
 * @param return $doi
 * @return int|RES_OK
 * @internal param item $item_id id of examined object
 * @internal param return $doi doi, according in id. return "" if item id not found.
 */
 ...
 * @internal param doi $dois array( use index 0 item only ) or doi value. if doi is NULL, search from item_id
 */
...
/** list index tree
 * @param int|XOONIPS_LISTINDEX_MODE_PUBLICONLY $mode
 * @param bool|true                             $assoc_array_mode
 * @return array|bool
 ...
 * @internal param XOONIPS_LISTINDEX_MODE_PUBLICONLY $mode <br>
 ...
 * @internal param true $assoc_array_mode : return index id assoc array, false: return normal array.
 */
function xnpListIndexTree($mode = XOONIPS_LISTINDEX_MODE_ALL, $assoc_array_mode = false)
{
    ...
    switch ($mode) {
        ...
        case XOONIPS_LISTINDEX_PUBLICONLY:
            ...
            break;
        case XOONIPS_LISTINDEX_PRIVATEONLY:
...
class XooNIpsItemLibraryObject
{
    ...
    /**
     * XooNIpsItemLibraryObject constructor.
     * @param $meta
     */
    ...
    public function getBasicInformationDetailBlock()
    {
        ...
        $related_to = $tpl->fetch('db:xoonips_detail_related_to.tpl');
        return array(
            'uid'               => array(
    ...
    public function getBasicInformationEditBlock($is_register)
    {
        ...
        $publcation_date   = $tpl->fetch('db:xoonips_publication_date.tpl');
        $publication_year  = $tpl->fetch('db:xoonips_publication_year.tpl');
        $publication_month = $tpl->fetch('db:xoonips_publication_month.tpl');
        $publication_mday  = $tpl->fetch('db:xoonips_publication_mday.tpl');
        // lang
        $tpl = new XoopsTpl();
        ...
        $lang = $tpl->fetch('db:xoonips_lang.tpl');
        // related to
        $tpl = new XoopsTpl();
        $tpl->assign('related_to', implode("\n", $basic['related_to']));
        if ($is_register) {
            $related_to = $tpl->fetch('db:xoonips_register_related_to.tpl');
        } else {
            $item_htmls = array();
            ...
            $related_to = $tpl->fetch('db:xoonips_edit_related_to.tpl');
        }
        ...
        return array(
            ...
            'title'             => array(
                ...
                'value' => sprintf('<textarea name="title" rows="3" cols="50" wrap="off" style="width:400px;">%s</textarea>',
                                   implode("\n", $basic['titles']))
            ),
            'keywords'          => array(
                'name'  => _MD_XOONIPS_ITEM_KEYWORDS_LABEL,
                'value' => sprintf('<input size="50" type="text" name="keywords" value="%s"/><br> Separate the words or phrases with commas.',
                                   implode(',', $basic['keywords']))
            ),
            'description'       => array(
                'name'  => _MD_XOONIPS_ITEM_DESCRIPTION_LABEL,
    ...
    public function getBasicInformationConfirmBlock($is_register)
    {
        ...
        $related_to = $tpl->fetch('db:xoonips_confirm_related_to.tpl');

        // description
        $description = $this->_getConfirmTemplateVars(_MD_XOONIPS_ITEM_DESCRIPTION_LABEL, $this->_item_basic_obj->get('description'),
                                                      $this->_item_basic_obj->getMaxLength('description'));

        // doi
        $doi = $this->_getConfirmTemplateVars(_MD_XOONIPS_ITEM_DOI_LABEL, $this->_item_basic_obj->get('doi'),
                                              $this->_item_basic_obj->getMaxLength('doi'));
    ...
    public function isCertifyRequired()
    {
        ...
        if (count($modified) == 1 && $modified[0] == _MD_XOONIPS_ITEM_INDEX_LABEL) {
            ...
            $index_item_link_objs   =  $index_item_linkHandler->getObjects();
        ...
        return ($indexHandler->getCount($criteria) > 0);
    }
...
class XooNIpsItemLibraryHandler
{
    public $_item_basicHandler;
    public $_xoops_usersHandler;
    public $_xoonips_usersHandler;
    public $_item_typeHandler;
    public $_item_keywordHandler;
    public $_item_titleHandler;
    public $_related_toHandler;
    ...
    {
        $this->_item_basicHandler    = xoonips_getOrmHandler('xoonips', 'item_basic');
        $this->_xoops_usersHandler   = xoonips_getOrmHandler('xoonips', 'xoops_users');
        $this->_xoonips_usersHandler = xoonips_getOrmHandler('xoonips', 'users');
        $this->_item_typeHandler     = xoonips_getOrmHandler('xoonips', 'item_type');
        $this->_item_titleHandler    = xoonips_getOrmHandler('xoonips', 'title');
        $this->_item_keywordHandler  = xoonips_getOrmHandler('xoonips', 'keyword');
        $this->_related_toHandler    = xoonips_getOrmHandler('xoonips', 'related_to');
        $this->_changelogHandler     = xoonips_getOrmHandler('xoonips', 'changelog');
    }
    ...
    public static function getInstance()
    {
        ...
        if (null === $instance) {
            $instance = new XooNIpsItemLibraryHandler();
        }
        return $instance;
    }
    ...
    public function create()
    {
        ...
        $meta['item_basic'] = $this->_item_basicHandler->create();
        $meta['item_basic']->set('uid', $uid);
        $meta['xoops_user']       = $this->_xoops_usersHandler->get($uid);
        $meta['xoonips_user']     = $this->_xoonips_usersHandler->get($uid);
        $meta['item_type']        = null;
    ...
    public function &get($item_id)
    {
        $ret                = false;
        $meta               = array();
        $meta['item_basic'] = $this->_item_basicHandler->get($item_id);
        ...
        $meta['item_type']        = $this->_item_typeHandler->get($item_type_id);
        $meta['item_title_arr']   = $this->_item_titleHandler->getTitles($item_id);
        $meta['item_keyword_arr'] = $this->_item_keywordHandler->getKeywords($item_id);
        $meta['related_to_arr']   = $this->_related_toHandler->getChildItemIds($item_id);
        $meta['changelog_arr']    = $this->_changelogHandler->getChangeLogs($item_id);
        $ret                      = new XooNIpsItemLibraryObject($meta);
        return $ret;
    }
        ...
        if (null !== $value) {
            ...
            foreach ($values as $num => $val) {
                ...
                if (isset($objs_old[$num])) {
                    ...
                    $obj = $this->_item_titleHandler->create();
                }
                $obj->set('title', $val);
        ...
        if (null !== $value) {
            ...
            foreach ($values as $num => $val) {
                ...
                if (isset($objs_old[$num])) {
                    ...
                    $obj = $this->_item_keywordHandler->create();
                }
                $obj->set('keyword', $val);
    ...
    public function insertBasicInformation($itemlib_obj)
    {
        ...
        $transaction->start();
        // basic information
        if (!$this->_item_basicHandler->insert($item_basic_obj)) {
        ...
        if (!$this->_item_titleHandler->updateAllObjectsByForeignKey('item_id', $item_id, $item_title_objs)) {
            $transaction->rollback();
            return false;
        }
        // keyword
        $item_keyword_objs =  $itemlib_obj->getKeywordObjects();
        if (!$this->_item_keywordHandler->updateAllObjectsByForeignKey('item_id', $item_id, $item_keyword_objs)) {
            $transaction->rollback();
            return false;
        }
        // related to
        $related_to_ids = array_unique(array_merge($itemlib_obj->getRelatedTo(), $itemlib_obj->getRelatedToCheck()));
        if (!$this->_related_toHandler->insertChildItemIds($item_id, $related_to_ids)) {
            $transaction->rollback();
            return false;
        }
        ...
        if ($certify_required_onupdate) {
            ...
            $index_item_linkHandler = xoonips_getOrmHandler('xoonips', 'index_item_link');
            $index_item_link_objs   =  $index_item_linkHandler->getByItemId($item_id, array(
                OL_PUBLIC,
                OL_GROUP_ONLY
            ));
            foreach ($index_item_link_objs as $index_item_link_obj) {
                $index_item_link_obj->set('certify_state', $certify_state);
                $index_item_linkHandler->insert($index_item_link_obj);
            }
        }

        // update item status
        $item_statusHandler = xoonips_getOrmHandler('xoonips', 'item_status');
        $item_statusHandler->updateItemStatus($item_id);
        ...
        if ($is_new) {
            ...
            $eventlogHandler->recordInsertItemEvent($item_id);
            // notification
