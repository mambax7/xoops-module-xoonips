...
<{/if}>
  ...
  <tr>
    ...
    <td class="<{cycle name="oddeven"}>"><{$detail.tool_type.display_value}></td>
  </tr>
  <tr>
    <td class="head"><{$smarty.const._MD_XNPTOOL_DEVELOPER_LABEL}></td>
    <td class="<{cycle name="oddeven"}>">
<{include file='db:xoonips_confirm_multiple_field.tpl' vars=$xnptool_developer}>
    </td>
  </tr>
  <tr>
...
<input type="hidden" name="tool_type" value="<{$detail.tool_type.value}>"/>
<input type="hidden" name="developer" value="<{$detail.developer.html_string}>"/>
