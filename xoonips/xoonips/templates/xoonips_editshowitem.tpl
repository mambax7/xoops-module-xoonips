...
<{foreach from=$item_types item=item_type}>
    <li style="float:left; padding: 0 1px; list-style:none; height:21px;"><a href="javascript:void(0);"
                                                                             onclick="return xoonips_editshowitem_select_itemtype(<{$item_type.item_type_id}>)"
                                                                             style="background: url( <{$xoops_url}>/modules/xoonips/images/tab_<{if $item_type.item_type_id == $item_type_id }>active<{else}>inactive<{/if}>_left.gif; ) no-repeat left top; height 21px; float:left; padding-left: 5px; text-decoration:none;"><span
            style="background: url( <{$xoops_url}>/modules/xoonips/images/tab_<{if $item_type.item_type_id == $item_type_id }>active<{else}>inactive<{/if}>_right.gif;) no-repeat right top; padding: 5px 5px 0 0;height: 21px; float:left; white-space: nowrap;"><{$item_type.name}><{if $item_type.navi.total != 0}>(<{$item_type.navi.total}>)<{/if}></span></a>
    </li>
    <{/foreach}>
</ul>
