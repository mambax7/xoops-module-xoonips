...
function b_xoonips_userlist_show()
{
    ...
    if (!$xsessionHandler->validateUser($uid, false)) {
        return false;
    }
    ...
    $position_objs =  $positionsHandler->getObjects($criteria);
    $positions     = array();
    foreach ($position_objs as $position_obj) {
        ...
        $user_objs =  $userHandler->getObjects($criteria, false, implode(',', $fields), false, $join_criteria);
        if (!empty($user_objs)) {
