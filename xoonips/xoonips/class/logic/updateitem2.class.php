...
class XooNIpsLogicUpdateItem2 extends XooNIpsLogic
{
    ...
    public function execute($vars, $response)
    {
        ...
        // check permission
        if (!$item_compoHandler->getPerm($item_id, $uid, 'write')) {
        ...
        // item_id -> old_item
        $old_item = $itemHandler->get($item_id);
        if (!$old_item) {
            $response->setResult(false);
            $error->add(XNPERR_NOT_FOUND);
            return false;
        ...
        foreach ($related_tos as $related_to) {
            ...
            } elseif (!$itemHandler->getPerm($related_item_id, $uid, 'read')) {
        ...
        if (!$this->isPublicationDateValid($response, $basic->get('publication_year'), $basic->get('publication_month'),
                                           $basic->get('publication_mday'), $detail_item_type->getRequired('publication_year'),
        ...
        $old_file_id_to_types
            = $this->extractFileIdToTypesFromItem($detail_item_type, $old_item);
        $new_file_id_to_types
            = $this->extractFileIdToTypesFromItem($detail_item_type, $item);
        ...
        for ($i = 0, $iMax = count($add_files); $i < $iMax; $i++) {
            ...
            if (!$fileHandler->insert($add_files[$i])) {
                $transaction->rollback();
                $response->setResult(false);
        ...
        $itemHandler->unsetNew($item);
        $basic->setVar('last_update_date', time(), true);
        if (!$basicHandler->insert($basic)) {
            $transaction->rollback();
    ...
    public function isFilesConsistent($error, $old_file_id_to_types, $new_file_id_to_types, $add_files, $delete_file_ids)
    {
        ...
        $all_file_ids = array_merge(array_keys($old_file_id_to_types), array_keys($new_file_id_to_types), array_keys($add_file_id_to_types),
                                    $delete_file_ids);
