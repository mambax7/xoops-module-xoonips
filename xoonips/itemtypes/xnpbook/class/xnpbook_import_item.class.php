...
/**
 * Class XNPBookImportItemHandler
 */
class XNPBookImportItemHandler extends XooNIpsImportItemHandler
{
    ...
    /**
     * XNPBookImportItemHandler constructor.
     */
    public function __construct()
    {
        parent::__construct();
    ...
    public function xmlStartElementHandler($parser, $name, $attribs)
    {
        ...
        switch (implode('/', $this->_tag_stack)) {
            ...
            case 'ITEM/DETAIL/FILE':
                ...
                $this->_book_pdf_file = $fileHandler->create();
                $this->_book_pdf_file->setFilepath($this->_attachment_dir . '/' . $attribs['FILE_NAME']);
                $this->_book_pdf_file->set('original_file_name',
                                           $unicode->decode_utf8($attribs['ORIGINAL_FILE_NAME'], xoonips_get_server_charset(), 'h'));
                $this->_book_pdf_file->set('mime_type', $attribs['MIME_TYPE']);
                $this->_book_pdf_file->set('file_size', $attribs['FILE_SIZE']);
    ...
    public function xmlEndElementHandler($parser, $name)
    {
        ...
        switch (implode('/', $this->_tag_stack)) {
            case 'ITEM/DETAIL':
                ...
                if (null === $detail->get('attachment_dl_limit')) {
                ...
                if (null === $detail->get('attachment_dl_notify')) {
                    if ($this->_detail_version === '1.00'
                        || $this->_detail_version === '1.01'
                    ) {
                        $detail->set('attachment_dl_notify', 0);
                    } else {
                        $this->_import_item->setErrors(E_XOONIPS_TAG_NOT_FOUND, ' no attachment_dl_notify tag '
            ...
            case 'ITEM/DETAIL/FILE':
                ...
                $fileHandler               = xoonips_getOrmHandler('xoonips', 'file');
                if (!$fileHandler->insert($this->_book_pdf_file)) {
                    $this->_import_item->setErrors(E_XOONIPS_DB_QUERY,
                                                   "can't insert attachment file:" . $this->_book_pdf_file->get('original_file_name')
                                                   . $this->_get_parser_error_at());
                }
                $this->_book_pdf_file = $fileHandler->get($this->_book_pdf_file->get('file_id'));
    ...
    public function import($item)
    {
        if ($item->getUpdateFlag()) {
            ...
            if ($item->hasBookPdf()) {
                $fileHandler = xoonips_getOrmHandler('xoonips', 'file');
                $clonefile   = $fileHandler->fileClone($book_pdf);
                $clonefile->setDirty();
