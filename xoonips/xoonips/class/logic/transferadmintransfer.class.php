...
/**
 * Class XooNIpsLogicTransferAdminTransfer
 */
class XooNIpsLogicTransferAdminTransfer extends XooNIpsLogicTransfer
{
    /**
     * XooNIpsLogicTransferAdminTransfer constructor.
     */
    ...
    public function execute_without_transaction($vars, $error)
    {
        ...
        foreach ($item_ids as $item_id) {
            ...
            if (!$eventlogHandler->recordTransferItemEvent($item_id, $index_id, $to_uid)) {
    ...
    public function add_user_to_groups($error, $uid, $gids)
    {
        foreach ($gids as $gid) {
            ...
            $groups_users_link        = $groups_users_linkHandler->create();
            $groups_users_link->set('gid', $gid);
            $groups_users_link->set('uid', $uid);
            $groups_users_link->set('is_admin', 0);
            if (false == $groups_users_linkHandler->insert($groups_users_link)) {
            ...
            // add event
            $eventlogHandler = xoonips_getOrmHandler('xoonips', 'event_log');
            if (!$eventlogHandler->recordInsertGroupMemberEvent($uid, $gid)) {
                $error->add(XNPERR_SERVER_ERROR, 'cannot insert event');
                return false;
            }
        }
