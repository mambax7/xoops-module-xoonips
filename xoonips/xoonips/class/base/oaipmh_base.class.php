...
class OAIPMH
{
     ...
     *
     * @param $attrs
     * @return string <ListMetadataFormats>
     *
     */
    public function ListMetadataFormats($attrs)
    {
        ...
        if (count($lines) == 0) {
            return $this->header() . $this->request($attrs) . $this->error('noMetadataFormats', '') . $this->footer();
        }
        ...
        return $this->header() . $this->request(array(
...
";
        ...
        return $this->header() . $this->request(array('verb' => 'Identify')) . "<Identify>\n" . $xml . "</Identify>\n" . $this->footer();
    }
...
';
    ...
    public function GetRecord($args)
    {
        ...
        if (count($args) == 3 && isset($args['verb']) && isset($args['metadataPrefix']) && isset($args['identifier'])) {
            ...
            if (strpos($args['identifier'], "\"")) {
                return $this->header() . $this->request($attrs) //.$this->error( 'badArguments', '' )
                       . $this->error('badArgument', '') . $this->footer();
            }
            // keio igarashi add 2007.10.01 to
        } else {
            return $this->header() . $this->request($attrs) . $this->error('badArgument', '') . $this->footer();
        }

        if (!isset($args['identifier'])) {
            return $this->header() . $this->request($attrs) . $this->error('badArgument', 'identifier is not specified') . $this->footer();
        }
        if (!array_key_exists($args['metadataPrefix'], $this->handlers)) {
            return $this->header() . $this->request($attrs) . $this->error('cannotDisseminateFormat', '') . $this->footer();
        }

        return $this->header() . $this->request($attrs) . $this->handlers[$args['metadataPrefix']]->GetRecord($args) . $this->footer();
    }
    ...
    public function ListIdentifiers($args)
    {
        ...
        if (isset($args['resumptionToken'])) {
            ...
            if (!$result || !array_search($args['resumptionToken'], $result)) {
                // keio igarashi add 2007.10.01 to
                return $this->header() . $this->request($attrs)
                       // keio igarashi add 2007.10.01 from
                       // add badArgument error
                       . $this->error('badArgument', '') // keio igarashi add 2007.10.01 to
                       . $this->error('badResumptionToken', '') . $this->footer();
            }
        ...
        return $this->header() . $this->request($attrs) . $this->handlers[$metadataPrefix]->ListIdentifiers($args) . $this->footer();
    }
    ...
    public function ListRecords($args)
    {
        ...
        if ((isset($args['from']) && !preg_match('/^[0-9]{4}-[0-9]{2}-[0-9]{2}/', $args['from']))
            || (isset($args['until'])
                && !preg_match('/^[0-9]{4}-[0-9]{2}-[0-9]{2}/', $args['until']))
        ) {
            return $this->header() . $this->request($attrs) . $this->error('badArgument', '') . $this->footer();
        }
        ...
        // keio igarashi add 2007.10.01 to
        return $this->header() . $this->request($attrs) . $this->handlers[$metadataPrefix]->ListRecords($args) . $this->footer();
    }
...
class OAIPMHHandler
{
     ...
     * @internal param OAI $id -PMH identifier
     */
    public function convertIdentifierFormat($id)
    {
        ...
        if ($parsed) {
            ...
            if ($parsed['nijc_code'] != $xconfigHandler->getValue('repository_nijc_code')) {
                return '';
            }
    ...
    public function oaipmh_header($identifier, $index_tree_list)
    {
        ...
        if (xnp_get_item_status($parsed['item_id'], $status) == RES_OK) {
            ...
            $sql     = 'SELECT link.index_id FROM ' . $xoopsDB->prefix('xoonips_index_item_link') . ' as link LEFT JOIN '
                       . $xoopsDB->prefix('xoonips_index') . ' as idx on link.index_id=idx.index_id ' . ' WHERE link.item_id='
                       . (int)$parsed['item_id'] . ' AND open_level=' . OL_PUBLIC;
...
class HarvesterHandler
{
    ...
    public function __construct($_parser)
    {
        ...
        xml_set_elementHandler($this->parser, 'startElementHandler', 'endElementHandler');
        xml_set_character_dataHandler($this->parser, 'characterDataHandler');
    }
...
function setResumptionToken($resumption_token, $metadata_prefix, $verb, $args, $last_item_id, $limit_row, $expire_date, $publish_date = null)
{
    ...
    $sql   = "INSERT INTO ${table} VALUES ('" . $myts->addSlashes($resumption_token) . "', '${metadata_prefix}', '${verb}', '"
             . $myts->addSlashes(serialize($args)) . "', ${last_item_id}, ${limit_row}, ${publish_date}, ${expire_date} )";
...
function getResumptionToken($resumption_token)
{
    ...
    $table       = $xoopsDB->prefix('xoonips_oaipmh_resumption_token');
    $sql
                 = "SELECT resumption_token, metadata_prefix, args, last_item_id, publish_date, expire_date, verb FROM ${table} WHERE resumption_token=\""
                   . $myts->stripSlashesGPC($resumption_token) . '"';
    $result      = $xoopsDB->query($sql);
