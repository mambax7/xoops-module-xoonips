...
define('_AM_XOONIPS_SYSTEM_RSS_FEED_URL_DESC',
       'XooNIps では RSS 1.0 (RDF), RSS 2.0, Atom 1.0 の 3種類のフィード形式をサポートしています．<br>以下のいずれかの URL を公開することにより公開アイテムの更新状況およびグループの新規作成状況をアナウンスできます．');
...
define('_AM_XOONIPS_SYSTEM_OAIPMH_HARVESTER_REPOSITORIES_DESC',
       'ハーベストするリポジトリの URL を指定します．入力はリポジトリごとに改行して下さい．<br>行の先頭に <strong>;</strong> を付けることでコメントアウトされます．');
// >> proxy configuration
...
define('_AM_XOONIPS_POLICY_ITEM_COMMENT_DESC',
       'アイテムへのコメント機能の設定を行います.<br>この機能は d3forum モジュールに依存しており，利用するにはまず d3forum をインストールしコメントを格納するフォーラムを作成する必要があります．');
