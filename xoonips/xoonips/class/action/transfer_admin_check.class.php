...
{
    /**
     * XooNIpsActionTransferAdminCheck constructor.
     */
    ...
    public function doAction()
    {
        ...
        $this->_view_params['item_ids_to_transfer']
            = $this->sort_item_ids_by_title($this->_formdata->getValueArray('post', 'checked_item_ids', 'i', false));

        $this->_view_params['child_items'] = $this->get_child_items($this->_formdata->getValue('post', 'from_uid', 'i', true),
                                                                    $this->_formdata->getValueArray('post', 'checked_item_ids', 'i', false));

        $this->_view_params['limit_check_result']
            = $this->get_limit_check_result($this->_formdata->getValue('post', 'to_uid', 'i', true),
                                            $this->_formdata->getValueArray('post', 'checked_item_ids', 'i', false));

        $this->_view_params['group_ids_to_subscribe']
            = xoonips_transfer_get_group_ids_of_items($this->_formdata->getValueArray('post', 'checked_item_ids', 'i', false));

        $this->_view_params['item_ids_transfer_disabled']
            = $this->get_transfer_disabled_item_ids($this->_formdata->getValue('post', 'from_uid', 'i', true),
                                                    $this->_formdata->getValueArray('post', 'checked_item_ids', 'i', false));

        $this->_view_params['can_not_transfer_items']
            = $this->get_untransferrable_reasons_and_items($this->_formdata->getValue('post', 'from_uid', 'i', true),
                                                           $this->_formdata->getValueArray('post', 'checked_item_ids', 'i', false));
    }
