...
<{include file='db:xoonips_confirm_multiple_field.tpl' vars=$xoonips_item.xnpmodel_creator}>
  ...
  <tr>
    ...
    <td class="<{cycle name="oddeven"}>"><{$model_data.value}></td>
  </tr>
  <tr>
...
<{if $editable }>
<{$basic.hidden}>
<input type="hidden" name="model_type" value="<{$xoonips_item.detail.model_type_value}>"/>
...
<input type="hidden" name="model_dataFileID" value="<{$xoonips_item.detail.model_data.file_id}>"/>
<{$preview.hidden}>
<{$readme.hidden}>
<{$rights.hidden}>
<{$index.hidden}>
