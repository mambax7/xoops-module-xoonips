...
function xnppresentationGetListBlock($item_basic)
{
    ...
    $xnppresentationHandler = xoonips_getOrmCompoHandler('xnppresentation', 'item');
    $tpl->assign('xoonips_item', $xnppresentationHandler->getTemplateVar(XOONIPS_TEMPLATE_TYPE_ITEM_LIST, $item_basic['item_id'], $myuid));

    // return as HTML
    return $tpl->fetch('db:xnppresentation_list_block.tpl');
}
...
function xnppresentationGetDetailBlock($item_id)
{
    ...
    $xnppresentationHandler = xoonips_getOrmCompoHandler('xnppresentation', 'item');
    $tpl->assign('xoonips_item', $xnppresentationHandler->getTemplateVar(XOONIPS_TEMPLATE_TYPE_ITEM_DETAIL, $item_id, $myuid));

    // return as HTML
    return $tpl->fetch('db:xnppresentation_detail_block.tpl');
}
...
function xnppresentationGetRegisterBlock()
{
    ...
    $tpl->assign('xnppresentation_creator',
                 xoonips_get_multiple_field_template_vars(xoonips_get_orm_from_post('xnppresentation', 'creator'), 'xnppresentation', 'creator'));

    // return HTML content
    return $tpl->fetch('db:xnppresentation_register_block.tpl');
}
...
function xnppresentationGetEditBlock($item_id)
{
    ...
    $tpl->assign('attachment_dl_limit',
                 xnpGetDownloadLimitationOptionEditBlock('xnppresentation', xnppresentationGetAttachmentDownloadLimitOption($item_id)));
    $tpl->assign('attachment_dl_notify',
                 xnpGetDownloadNotificationOptionEditBlock('xnppresentation', xnppresentationGetAttachmentDownloadNotifyOption($item_id)));
    $tpl->assign('detail', $detail);
    $tpl->assign('readme', $readme);
    $tpl->assign('rights', $rights);
    $tpl->assign('presentation_type', xnppresentationGetTypes());
...
function xnppresentationGetConfirmBlock($item_id)
{
    ...
    // trim strings
    if (xnpHasWithout($basic) || xnpHasWithout($readme) || xnpHasWithout($rights) || xnpHasWithout($detail) || xnpHasWithout($preview)
        || xnpHasWithout($presentation_file)
    ...
    // return HTML content
    return $tpl->fetch('db:xnppresentation_confirm_block.tpl');
}
...
function xnppresentationGetAdvancedSearchBlock($search_var)
{
    ...
    // return HTML content
    return $tpl->fetch('db:xnppresentation_search_block.tpl');
}
...
{
    return xnpGetTopBlock($itemtype['name'], $itemtype['display_name'], 'images/icon_presentation.gif', _MD_XNPPRESENTATION_EXPLANATION,
                          'xnppresentation_presentation_type', xnppresentationGetTypes());
}
