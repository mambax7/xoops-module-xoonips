...
class ListRecordsHandler extends HarvesterHandler
{
    ...
    /**
     * ListRecordsHandler constructor.
     *
     * @param $_parser
     * @param $_baseURL
     * @param $_metadataPrefix
     */
    ...
    public function endElementHandler($parser, $name)
    {
        ...
        if (end($this->tagstack) === 'RESUMPTIONTOKEN') {
            ...
            $repositories = $repositoryHandler->getObjects($criteria);
            if (!$repositories) {
                $this->search_text = array();
                array_pop($this->tagstack);
            ...
            $metadata = $metadataHandler->getByIdentifier($this->identifier);
            if ($metadata && $this->delete_flag) {
                $this->deleteMetadataFields($metadata->get('metadata_id'));
                $metadataHandler->delete($metadata);
                $this->search_text = array();
                array_pop($this->tagstack);
            ...
            if (!$metadata) {
                $metadata = $metadataHandler->create();
            }
            ...
            $metadata->set('creator',
                           count($this->_creator) > 0 ? mb_strcut($unicode->decode_utf8($this->_creator[0], xoonips_get_server_charset(), 'h'), 0,
                                                                  255) : '');
            ...
            $metadata->set('link',
                           count($this->_resource_url) > 0 ? $unicode->decode_utf8($this->_resource_url[0], xoonips_get_server_charset(), 'h') : '');
            ...
            $result = $metadataHandler->insert($metadata, true);
            if (!$result) {
