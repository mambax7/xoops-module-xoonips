...
function php_compat_sha256($str, $raw_output = false)
{
    ...
    for ($i = 0; $i < strlen($str); $i += 64) {
        ...
        for ($j = 16; $j < 80; ++$j) {
            $s0 = php_compat_sha256_rotr_helper($w[$j - 15], 7) ^ php_compat_sha256_rotr_helper($w[$j - 15], 18) ^ php_compat_sha256_shr_helper($w[$j
                                                                                                                                                   - 15],
                                                                                                                                                3);

            $s1 = php_compat_sha256_rotr_helper($w[$j - 2], 17) ^ php_compat_sha256_rotr_helper($w[$j - 2], 19) ^ php_compat_sha256_shr_helper($w[$j
                                                                                                                                                  - 2],
                                                                                                                                               10);

            $w[$j] = php_compat_sha256_add32_helper(php_compat_sha256_add32_helper(php_compat_sha256_add32_helper($w[$j - 16], $s0), $w[$j - 7]),
                                                    $s1);
        }
        ...
        for ($j = 0; $j < 64; ++$j) {
            $s1 = php_compat_sha256_rotr_helper($e, 6) ^ php_compat_sha256_rotr_helper($e, 11) ^ php_compat_sha256_rotr_helper($e, 25);

            $ch = ($e & $f) ^ (~$e & $g);

            $s0 = php_compat_sha256_rotr_helper($a, 2) ^ php_compat_sha256_rotr_helper($a, 13) ^ php_compat_sha256_rotr_helper($a, 22);

            $maj = ($a & $b) ^ ($a & $c) ^ ($b & $c);

            $t1 = php_compat_sha256_add32_helper(php_compat_sha256_add32_helper(php_compat_sha256_add32_helper(php_compat_sha256_add32_helper($h,
                                                                                                                                              $s1),
                                                                                                               $ch), $k[$j]), $w[$j]);
