...
{
    /**
     * XooNIpsOrmIndexGroupIndexLink constructor.
     */
...
class XooNIpsOrmIndexGroupIndexLinkHandler extends XooNIpsTableObjectHandler
{
    ...
    public function getObjectsByItemId($item_id)
    {
        ...
        foreach ($index_item_linkHandler->getObjects($criteria, false, '', true, $join) as $row) {
    ...
    public function requireToMakePublic($to_index_id, $group_index_ids)
    {
        ...
        foreach ($group_index_ids as $group_index_id) {
            ...
            if ($index_group_index_linkHandler->getObjects($criteria1)) {
    ...
    public function notifyMakePublicGroupIndex($to_index_ids, $group_index_ids, $notification_name)
    {
        ...
        $moderator_uids = $memberHandler->getUsersByGroup($configHandler->getValue('moderator_gid'));
        ...
        if ($notification_name === 'group_item_certify_request') {
            ...
            // to all moderator
            $result = $notificationHandler->triggerEvent('administrator', 0, $notification_name, $tags, $moderator_uids);
            ...
            $target_users = array_unique(array_merge($moderator_uids, $xgroupHandler->getUserIds($group->get('gid'), true)));
            $notificationHandler->triggerEvent('user', 0, $notification_name, $tags, $target_users);
        }
    }
