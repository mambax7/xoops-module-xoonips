...
$total = $admin_fileHandler->getCountFiles();
if ($num < 0 || $num > $total) {
    die('fatal error : invalid \'num\' parameter');
}
$file_id = $admin_fileHandler->getFileIdByCount($num);
if ($file_id === false) {
    die('fatal error : file id not found');
}

if ($mode === 'info') {
    $admin_fileHandler->updateFileInfo($file_id);
} elseif ($mode === 'index') {
    $admin_fileHandler->updateFileSearchText($file_id, false);
} else {
