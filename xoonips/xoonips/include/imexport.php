...
function xnpExportIndex($fhdl, $index_id, $recurse)
{
    ...
    $xml = array();
    if ($recurse) {
        $res = xnp_get_indexes($xnpsid, $index_id, array(
            'orders' => array(
                array(
                    'sort_number',
                    0,
...
{
    $file
        = xnpGetFileInfo('t_file.file_id, t_file_type.name, t_file.original_file_name, t_file.file_size, t_file.mime_type, t_file.thumbnail_file, t_file.caption',
                         "item_id = ${item_id} and is_deleted=0", $item_id);
    ...
    foreach ($file as $f) {
        $file = array();
        list($file['file_id'], $file['file_type_name'], $file['original_file_name'], $file['file_size'], $file['mime_type'], $file['thumbnail_file'],
            $file['caption'])
            = $f;
 ...
 * @internal param reference $errmsg recieve error message
 */
function xnpImportIndex($str, $parent_index_id, $id_table, $errmsg)
{
    ...
    xml_set_elementHandler($parser, '_xoonips_import_startElement', '_xoonips_import_endElement');
    xml_set_character_dataHandler($parser, '_xoonips_import_CharacterData');
...
function _xoonips_import_startElement($parser, $name, $attribs)
{
    ...
    if (array_key_exists('basic', $parser_hash[$parser]) && array_key_exists('ID', $parser_hash[$parser]['basic'])
        && !in_array($parser_hash[$parser]['basic']['ID'], $parser_hash[$parser]['accept_id'])
    ) {
        return;
    }
...
function _xoonips_import_indexcheckCharacterData($parser, $data, $parser_hash)
{
    ...
    switch ($tags) {
        case '/INDEXES/INDEX/TITLE':
            $parser_hash['indexes'][count($parser_hash['indexes']) - 1]['titles'][count($parser_hash['indexes'][count($parser_hash['indexes'])
                                                                                                                - 1]['titles']) - 1]
                .= $data;
            break;
    }
...
function _xoonips_import_index($parent_index_id, $indexes, $id_table)
{
    ...
    foreach ($indexes as $index) {
        foreach ($index['titles'] as $k => $title) {
            list($index['titles'][$k], $dummy) = xnpTrimString($unicode->decode_utf8($title, xoonips_get_server_charset(), 'h'), $lengths['title'],
                                                               'UTF-8');
        }
        $child = array();
